...
本仓库当前服务于一款围绕“唯一核心卡”展开身份选择、探索、资源竞争、交易与关键冲突解决的 2D 系统型游戏构思。仓库不直接实现游戏代码，而是提供一套可追踪、可复盘、可扩展的游戏构思系统；战斗系统当前处于重新设计阶段。
...
一款围绕唯一核心卡展开身份选择、探索、资源竞争、交易与关键冲突解决的 2D 系统型游戏。唯一核心卡继续承担玩家身份与长期资产职责；新战斗形式、核心动作和核心卡参与方式等待 GDD-0 重新确认。
...
- 正式主线是“唯一核心卡驱动的整体系统构思”；旧成语组合战斗方案已归档，不再是实现基线。
- 当前阶段优先从 Raw Idea 重新建立战斗 GDD-0；依赖旧战斗接口的地图灰盒和生产计划暂时 `Parked`。
