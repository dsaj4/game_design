...
方向索引位于P/directions/，仅负责导航、背景、关系与资格映射，不是正式素材或Proposal。用户明确指定当轮GDD时，可作为限定补充来源，并记录提交/哈希；不能因此原地更新已激活包。用户仅选一个方向时只读取该方向所需正文，跨方向比较须有相应任务范围。历史运行全文放runs/；主系统玩法回写要求保持。
