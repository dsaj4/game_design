...
| 模块 | 初学者解释 |
| --- | --- |
| 唯一核心卡 | 每个玩家持有一张独特核心卡，用它建立长期身份和资产关系 |
| 共享世界层 | 地图、探索、资源、交易和遭遇制造长期目标与冲突来源 |
| 战斗层（重新设计中） | 仍负责解决关键冲突并向地图返回结果；具体形态、动作与接口为 `Unknown` |
| 动态平衡层 | 尽量不改核心卡本体，通过外围参数和未来确认的战斗外部参数调节环境 |

当前关键不是继续实现旧组合规则，而是重新确认玩家为什么进入战斗、反复做什么、获得什么反馈，以及唯一核心卡如何参与。旧“成语组合 + 共享战斗能量”只保存在 `archive/` 中作为历史证据。
