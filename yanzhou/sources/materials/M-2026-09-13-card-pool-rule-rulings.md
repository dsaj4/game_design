...
状态：Qualified GDD Material / Accepted，G002-CORE-019。日期：2026-09-13。证据：Hypothesis；新规则未运行测试。

## 素材控制

| 字段 | 内容 |
| --- | --- |
| 素材 ID | MAT-2026-09-13-card-pool-rule-rulings |
| 素材类型 | System / Mechanic / Constraint |
| 来源 inbox | [推荐原文与用户确认](../inbox/2026-09-13-card-pool-rule-recommendations.md) |
| 资格确认日期 | 2026-09-13 |
| 确认方式 | grill-with-docs：核对现用规则、55项卡面和K01–12；用户明确“按推荐稿确认” |
| 当前证据状态 | Hypothesis；规则选择已完成，参数、全组合与体验未验证 |
| 负责人 | 用户决定，设计agent维护 |

## 规范化表述

用户采用RC01–12完整推荐：首次来源决定持久特性、有限完整复诵、蓄势兑现、单次结果替换、一层即时触发、逐刻元素过程、明确支付和自耗动作。下列规则为当前权威文本；原推荐留作历史，未指定的数值不补默认。用户随后另行采用S2整池首版选用（G002-SCOPE-003）及空间／出生（G002-CORE-020／021），见[首版与空间素材](M-2026-09-13-first-release-content-scope.md)；其余BF转化、IN-C装配获取与FAT-C仍未整包采纳。
...
- 对象：S2／E3共同规则及被点名卡片的局部例外。
- 目标：未来全游戏GDD的语法、时序、状态、元素和组合章节；按用户最新要求，当前先关闭设计缺口，暂停新GDD写作。
- 完成度：GDD-1规则素材；不是参数完整的GDD-2或开发放行。

## 玩家处境、行为与反馈

玩家战前分配词卡并设置法杖，比较创建来源、复诵节拍、状态消耗和结果替换。战中可解释下一次追加、蓄满兑现、元素补层、原结果被替代及有限响应的原因。预期价值为少记账、先后可推导、资源不会被隐式复制；实际可读性和策略宽度仍待验证。

## 与当前构思的关系

复用实体卡、共享槽、SW02-A固定名单、GR联合支付、ST01–04刻末状态。明确改变：双律环的局部选敌权限；生成类形容词适配；指令芯跨操控杖；自耗动作完成许可；S2／E3派生时点。元素逐刻子事件不成为无限完整施法。旧BF独立寿命、旧三刻状态实验和旧延后派生不能覆盖本版。

## 机制—行为—体验假设

| Mechanics | Dynamics | Aesthetics |
| --- | --- | --- |
| 首次来源、单次替换和一次实付 | 调整先后与消耗对象，主动决定是否清空重建 | 规则可预测 |
| 完整复诵竞争共享槽、正常循环保留 | 安排起点与法杖顺序，保护兑现机会 | 节拍取舍 |
| 一层即时触发、邻近请求合并 | 使用明确组合，观察有限结果 | 反馈靠近原因 |
