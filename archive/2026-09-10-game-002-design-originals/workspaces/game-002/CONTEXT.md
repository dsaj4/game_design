...
当前规则版本：`Core Concept v0.5 / Stable Design Baseline`。本词汇表、总控和旧素材出现冲突时，以[核心构思](game-design-workflow/core-concept.md)、最新 Accepted 决策和[战前构句、循环法术与法杖设置适配](game-design-workflow/draft-changes/D-2026-09-09-prebattle-spell-wand-assembly.md)为准；旧素材保留其来源原文，不自动代表当前首版规则。
...
每场战斗前把实体词卡按组合分配给多条循环法术，并把首次启动时刻放入有限窗口；每根法杖承载一条法术，法杖及其镶嵌配置确定范围与战前固定的合法对象；启动后各法术按自身周期运行，释放机会按每刻一个的共享槽竞争。它是核心构思 v0.5 的编排机制，具体容量、法杖和镶嵌参数仍待验证。
...
由实体词卡组装、在首次启动后按自身冷却与释放周期重复尝试的法术。基础模式中不提供战中通用重排；被同刻覆盖只损失本次释放，受到生命伤害时只打断当时正在冷却的法术，后续周期保持。目标失效时不结算并跳过。具体效果和特殊法术范围后置。
...
战斗中不使用牌组、手牌或补牌；词卡保留在战斗外，玩家根据库存于下一场战斗前构筑循环法术并设置法杖。跨局继承沿用新局资源规则，具体例外后置。
来源见[时间背包自走棋规则确认记录](game-design-workflow/draft-changes/D-2026-09-09-timeline-backpack-v03-confirmation.md)。

**法术生成收益**：

金币由基础收益、战斗时间额外奖励和特殊情况下的法术效果产生；卡牌由法术效果产生。战后金币与卡牌一起结算，整体领取或放弃。
来源同[时间背包自走棋规则确认记录](game-design-workflow/draft-changes/D-2026-09-09-timeline-backpack-v03-confirmation.md)。
...
战斗开始前，玩家使用战外词卡库存中的实体词卡，依此前战中构句的词性、句式、语义兼容、执行者、操作内容、受术目标、状态引用和法术类型规则组成一条完整法术。一个组合只对应一条法术；每张实体词卡在本次配置中最多归属于一条循环法术。构句不推进时间，不经过抽牌、手牌或补牌；构成的法术会在后续战斗中自动循环。

**设置法杖**：

在战前把每条循环法术绑定到一根法杖，并确定法杖及法术的前后顺序。基础规则中每根法杖承载一条循环法术；靠后的法术在同刻覆盖靠前法术。法杖可以配置镶嵌。

**法杖镶嵌**：

法杖上的构筑配置层，承接此前全局道具的效果设计方向，未来可决定施法范围、特殊效果或其他明确修正。镶嵌槽位、资格、替换、范围参数和具体效果均为 `Unknown`；旧全局道具的直接全局生效和无装备槽规则不自动适用于当前法杖框架。

## 支撑与历史术语（不覆盖 v0.5）

以下条目来自 v0.1–v0.3 的合格素材和候选系统。它们继续保留作为来源、研究和后续扩展词汇；其中涉及战中牌组、手牌、补牌、抽弃循环、战中逐句施法或“本局卡组”的内容，不属于当前稳定首版规则。
