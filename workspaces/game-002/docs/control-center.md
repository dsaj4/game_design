...
下一步：决定某类无合格候选时的处理。建议选择休整分支前检查能否组成完整卡包；若某类无可选词，则本次不提供拿牌分支，保留恢复。[候选不足评议](../game-design-workflow/idea-inbox/2026-09-05-yanzhou-core-combat.md#当前整体机制评议某类没有可选词时的休整)仍为 Raw Idea / Unqualified。该建议避免放弃恢复后才发现无完整卡包，代价是其他类别尚有词时也暂时不能拿牌；具体词池与恢复量后置。
