...
最新方向：用户要求设置卡组下限，防止抽牌堆与弃牌堆同时为空；双空堆跳过补牌建议未采纳。新下限方案为 Raw Idea / Unqualified，尚未写入合格素材。

下一步：使用 grill-with-docs 确认本场移除是否也受安全下限约束，再核对手牌、施法区及超限窗口的占用。具体数值后置；预留不等于实现。
