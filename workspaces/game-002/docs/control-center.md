...
下一步：澄清临时副本是否默认正常弃置、参与洗回。推荐采用，使复制可持续影响本场组件分布；[循环候选](../game-design-workflow/idea-inbox/2026-09-05-deck-vocabulary-cycle.md#当前构筑评议临时副本是否参与正常循环)仍待确认，复制对象、进入位置与费用、其他数值及界面后续再明确。
