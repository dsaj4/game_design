...
下一步：澄清副本生成后的初始进入位置。推荐基础复制默认置于抽牌堆顶部，按原补牌节拍入手；[位置候选](../game-design-workflow/idea-inbox/2026-09-05-deck-vocabulary-cycle.md#当前构筑评议临时副本的初始进入位置)仍待确认。复制对象、费用、多副本内部顺序、其他数值及界面后续再明确。
