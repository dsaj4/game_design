...
下一步：使用 grill-with-docs 澄清临时副本能否使本场同名数量超过三张。推荐允许，候选见[临时副本与上限评议](../game-design-workflow/idea-inbox/2026-09-05-deck-vocabulary-cycle.md#当前构筑评议临时副本能否超过三张)，仍待用户确认；复制对象、流转与费用、其他数值及界面后续再明确。
