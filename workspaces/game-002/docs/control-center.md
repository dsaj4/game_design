...
下一步：决定休整卡包的词类分布。建议基础版主语、谓语、宾语各一张，在各自合格词卡中随机产生；[词类分布候选](../game-design-workflow/idea-inbox/2026-09-05-yanzhou-core-combat.md#当前整体机制评议休整卡包的词类分布)为 Raw Idea / Unqualified。该建议覆盖类别选择，代价是减少同类候选比较；不保证语义兼容，过滤后词种不足的处理仍须另议。
