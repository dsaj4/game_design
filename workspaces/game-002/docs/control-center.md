...
| 类型、方向、目标玩家 | Accepted / Core Concept v0.5 Stable Design Baseline：单人、自走棋式自动战斗；战前构句、循环法术构筑、法杖设置与时间编排；目标玩家和平台仍 Unknown |
| 原始想法记录、素材、正式 GDD | 9 / 40 / 0（9 份 inbox、40 份 Qualified GDD Material、尚无正式 GDD；素材仍为 Hypothesis，当前规则以 v0.5 核心和决策记录为准） |
| 提案、评估、正式玩法决定 | 无 |
| 原型与代码 | 已有离线牌库与战斗规则模拟，属于旧战斗流程证据；当前自走棋规则尚无可玩原型或游戏实现任务 |
| 生产阶段、排期 | 未确定 |

当前核心：[Core Concept v0.5](../game-design-workflow/core-concept.md)，Accepted / Stable Design Baseline；证据状态 Hypothesis，尚未通过完整对局和玩家体验验证。

当前规则优先级：`core-concept.md` > `decision-log.md` 中最新 Accepted 决定 > [战前构句、循环法术与法杖设置适配](../game-design-workflow/draft-changes/D-2026-09-09-prebattle-spell-wand-assembly.md) > [时间背包自走棋规则确认记录](../game-design-workflow/draft-changes/D-2026-09-09-timeline-backpack-v03-confirmation.md) > 其他 Qualified GDD Material。早期素材保留原始澄清和验证依据；其中与 v0.5 冲突的战中牌组、手牌、补牌、逐句施法和“本局卡组”表述不属于当前首版规则。

## 当前稳定基线

1. 战斗是自走棋式自动战斗。战斗中不使用牌组、手牌或补牌，也不进行战中构句；战斗外保留词卡库存，战前依次构句、构成循环法术、设置法杖。
2. 战前构句沿用此前战中构句的词性、句式、语义兼容、执行者/操作内容/受术目标、状态引用和法术类型规则；一个组合对应一条法术。玩家可使用完整战外词卡库存装配，不抽牌；每张实体词卡在本次配置中只归属一条循环法术。
3. 每根法杖只承载一条循环法术，每条循环法术绑定一根法杖。战前设置所有法杖和法术的顺序，法杖镶嵌承接旧全局道具的效果方向；具体槽位、范围、特殊效果和修正规则仍 Unknown。
4. 循环周期等于冷却加释放。冷却从第 0 刻开始时，4 刻冷却、1 刻释放的法术在第 4 刻首次释放；启动后按自身周期循环。
5. 所有法术共享每刻一个释放槽，后置法术覆盖同刻的前置法术；被覆盖的本次释放无效果、无释放特效，后续周期不变。
6. 场景是简易长方形地图。第一阶段敌人和环境对象不移动，法杖及其镶嵌所确定范围内的合法对象在战前固定；释放时失效目标不结算并跳过，但失效目标仍播放释放特效，法术继续处理其他合法对象。
7. 生命值实际受伤才会打断法术，而且只打断当时正在冷却的法术；护甲完全抵伤不打断，已完成并结算的法术不被追溯打断。
8. 同刻顺序为法术效果、敌人攻击、环境变化、状态效果；胜利成立后立即中断全部后续处理，包括法术。
9. 环境对象有实际位置，可被法术作用，但没有生命值和攻击行为，例如大树、溪流。
10. 金币由基础收益、战斗时间额外奖励和特殊法术效果产生；卡牌由法术效果产生。战后金币与卡牌一起整体领取或放弃。
11. 具体词义、效果、状态承接、法杖基础属性、镶嵌槽位与效果、词卡容量、奖励数量和动态目标扩展仍属于后续设计或验证，不得由旧素材自行补齐。

以上基线是当前稳定的设计规则版本，不代表数值平衡、体验或实现已经验证。任何改动须新建 Draft Change，并同步决策记录。
...
当前改动（2026-09-07）：数值框架 v0.1 已确认并晋级第40份素材；新增测试卡表及[首场牌库模拟报告](../research/06-prototype-insights/2026-09-07-simple-spell-opening-battle.md)。主例38T/92血，256种子两策略共512次独立首战；模型内检查通过，测试词义、力量值与实际玩法仍待评议。

下一步：优先评议测试词义复用与攻击组件密度，再单独对照力量数值或牌组比例；随后扩大敌方节拍、多敌与后续路线。保留本次种子和数据，不以样本全胜宣称整局平衡。
