...
下一步：决定同一卡包三张候选是否互不重名。建议同包提供三个不同词卡选项，同时允许不同卡包继续出现未满三张的已有词；[重名候选评议](../game-design-workflow/idea-inbox/2026-09-05-yanzhou-core-combat.md#当前整体机制评议同一卡包候选是否重名)仍为 Raw Idea / Unqualified。词类分布、卡池权重、过滤后候选不足、主动重抽与补偿等另议，数值、复制能力及界面继续后置。
