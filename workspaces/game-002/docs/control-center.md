...
| 类型、方向、目标玩家 | Accepted / Core Concept v0.4 Stable Design Baseline：单人、自走棋式自动战斗；战前词卡构筑、法杖设置与时间编排；目标玩家和平台仍 Unknown |
| 原始想法记录、素材、正式 GDD | 9 / 40 / 0（9 份 inbox、40 份 Qualified GDD Material、尚无正式 GDD；素材仍为 Hypothesis，当前规则以 v0.4 核心和决策记录为准） |
| 提案、评估、正式玩法决定 | 无 |
| 原型与代码 | 已有离线牌库与战斗规则模拟，属于旧战斗流程证据；当前自走棋规则尚无可玩原型或游戏实现任务 |
| 生产阶段、排期 | 未确定 |

当前核心：[Core Concept v0.4](../game-design-workflow/core-concept.md)，Accepted / Stable Design Baseline；证据状态 Hypothesis，尚未通过完整对局和玩家体验验证。

当前规则优先级：`core-concept.md` > `decision-log.md` 中最新 Accepted 决定 > [时间背包自走棋规则确认记录](../game-design-workflow/draft-changes/D-2026-09-09-timeline-backpack-v03-confirmation.md) > 其他 Qualified GDD Material。早期素材保留原始澄清和验证依据；其中与 v0.4 冲突的战中牌组、手牌、补牌、逐句施法和“本局卡组”表述不属于当前首版规则。

## 当前稳定基线

1. 战斗是自走棋式自动战斗。战斗中不使用牌组、手牌或补牌，也不进行战中构句；战斗外保留词卡库存，战前从库存构筑循环法术并设置法杖。
2. 每根法杖只承载一条循环法术。战前设置所有法杖和法术的顺序，安排每条法术第一次冷却在第 0–10 刻开始。
3. 循环周期等于冷却加释放。冷却从第 0 刻开始时，4 刻冷却、1 刻释放的法术在第 4 刻首次释放；启动后按自身周期循环。
4. 所有法术共享每刻一个释放槽，后置法术覆盖同刻的前置法术；被覆盖的本次释放无效果、无释放特效，后续周期不变。
5. 场景是简易长方形地图。第一阶段敌人和环境对象不移动，法杖范围内的合法对象在战前固定；释放时失效目标不结算并跳过，但失效目标仍播放释放特效，法术继续处理其他合法对象。
6. 生命值实际受伤才会打断法术，而且只打断当时正在冷却的法术；护甲完全抵伤不打断，已完成并结算的法术不被追溯打断。
7. 同刻顺序为法术效果、敌人攻击、环境变化、状态效果；胜利成立后立即中断全部后续处理，包括法术。
8. 环境对象有实际位置，可被法术作用，但没有生命值和攻击行为，例如大树、溪流。
9. 金币由基础收益、战斗时间额外奖励和特殊法术效果产生；卡牌由法术效果产生。战后金币与卡牌一起整体领取或放弃。
10. 具体词义、效果、状态承接、法杖范围参数、词卡容量、奖励数量和动态目标扩展仍属于后续设计或验证，不得由旧素材自行补齐。
