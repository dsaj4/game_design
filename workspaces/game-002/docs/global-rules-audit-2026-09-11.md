...
日期：2026-09-11。Project ID：game-002。状态：Documentation Review Completed；玩法与数值验证Deferred。审查基线提交：fb75812263f76992c7ebab5c66eb857a001abb63，Core Concept v0.6／G002-CORE-014。本页是审查记录，不是新规则采纳或测试报告。

## 结论

现有战前配置、实体分配、循环释放、引用名单、护甲、类型及战后收益骨架可继续支撑卡牌与流派草拟。本轮找到四处可依据现有决定直接修正的表述，已同步；另外登记12组歧义、候选边界或已知输入依赖。不能据此声称全局规则已完全闭合或平衡已通过。
...
- 通过grill-with-docs进行领域与规则对照。新建议来源保存在[本轮inbox](../game-design-workflow/idea-inbox/2026-09-11-global-rules-review.md)，仍为Raw Idea / Unqualified；下列“推荐”均不直接成为GDD依据。
...
DC01是明确规则冲突；DC02–04是与较新已采纳范围衔接不足的表述。修正按G002-CORE-011／012、SW02-A和R08／25执行，没有引入第五法术类型、生成物新权限或新时序规则；核心正文和决策采纳状态不变。

## 待明确项总表

以下12组不等于12项玩法缺陷。GR01–07为通用边界的未闭合部分；GR08–10是已登记内容／疲劳候选依赖；GR11是候选验收范围的张力；GR12是实际评测资源输入不足。

| ID | 主题 | 性质 | 最晚处理时点 |
| --- | --- | --- | --- |
| GR01 | 数值从词到整句如何得到 | 未闭合的通用口径 | 新词卡进入可评测候选前 |
| GR02 | 冷却区间、取消与改期后的续排 | 时序边界缺口 | 时间类卡进入评测前 |
| GR03 | 多个执行者与目标、筛选权限怎样组合 | 语义配对与成本缺口 | 多角色／复杂条件卡进入评测前 |
| GR04 | 零数量、状态不存在和被禁止效果的区别 | 状态边界缺口 | 状态数量操作进入评测前 |
| GR05 | 一次费用、对象费用和生命支付是否能完整支付 | 成本边界缺口 | 带额外成本的卡进入评测前 |
| GR06 | 生命归零到完整事件结束之间如何处理 | 生命与事件边界缺口 | 多步骤／死亡收益卡进入评测前 |
| GR07 | 同阶段新事件与额外触发的次序和终止 | 事件安排缺口 | 反应／触发型流派进入评测前 |
| GR08 | 生成物在哪出现、可操作到何时 | 已知的内容依赖 | 首张元素／生成物卡进入评测前 |
| GR09 | 产出资格、次数和失败尝试如何记账 | 已知的资源词义缺口 | 首张产卡／产金卡进入评测前 |
| GR10 | 疲劳方向已定，完整执行仍是候选 | 已知的候选依赖 | 采用疲劳的下一批测试冻结前 |
| GR11 | 无秒杀的适用范围和跨战残血风险 | 候选验收口径存在张力 | 评测报告据此决定卡牌优劣前 |
| GR12 | 库存、范围、成长和获取资格尚需明确输入 | 已知的评测输入缺口 | 每批可比较样本冻结前 |
...
触发情境：现有两句分别把伤害量放在动词、护甲生成量放在名词；新组合的r仍逐句待设计。固定削弱使原本非负的量越过0时，最终截断点也未写全。

推荐处理（未采纳）：每个词条声明自己贡献的参数及适用角色，操作语义声明整句效果与r的计算方法，不用整句名称暗设加成。推荐非负数量统一在完整修正后取整并截至0；τ、c、r、状态周期的整数域及零值许可另列，不能只写P为正。
...
触发情境：规则给了计划释放点及成功后续排，但未用统一区间说明冷却开始刻的敌攻是否打断；当前冷却被改期后又被打断／覆盖，如何产生下一轮缺少完整表。

推荐处理（未采纳）：推荐以半开区间明确冷却与释放：冷却包含开始、排除释放开始；取消的机会保留当时已确定的名义释放段与后续周期。改期只改当前机会，按其新的名义释放结束点续基础冷却；取消标记不因再次改期复活。同一过程多次改期按公开事件顺序处理。

影响与边界：未改期的两句参照仍可用；不能把本建议冒充R23已经逐项回答。
...
触发情境：同一法术能让不同角色各自选一组对象，但“2个执行者、3个目标”是2次、3次还是6次作用没有统一答案；选择配置能否免费添加新词义也会改变成本。

推荐处理（未采纳）：首批优先单执行者；每种操作声明角色数量、同宿主／一对一／其他配对许可及去重单位，未声明不自动作全组合。同次名单固定不等于角色配对已经定义。筛选只配置已获词义的参数；新增语义仍由实体词卡承担，并明示条件字段和限量优先级。
...
触发情境：状态被消耗至0后是否还保留身份、计时与存在条件，文档没有统一判定；“生成0”“读取0”“解除不存在状态”也不能仅按数值相同判断成功。

推荐处理（未采纳）：每个状态声明零数量是否意味着清除，推荐数量型状态耗尽即清除；有无型状态独立声明。生成、读取可允许合法零值；消耗不足、对象不存在、规则禁止分别按合法性处理。先判断是否合法，再决定实际量，不用“没有实际收益”统一否决成功。
...
触发情境：一次性费用和首个对象费用分别足够、合计却不足时，应在哪一步判失败？支付本身消耗必需状态或恰好耗尽执行者生命，也缺少明确处理。

推荐处理（未采纳）：推荐先联合检查本次一次性费用与首个可执行对象的全部固定费用，满足后一起提交，再逐对象处理后续费用；不允许先收一次费却没有任何可结算对象。默认生命支付后须存活，若设计自毁则专门声明；不得把费用不足当合法零值或自动退款已完成效果。
...
触发情境：法术第一步令单位归零，后续步骤还引用它时，“当前合法性”与“完整事件后才判击败”尚缺清晰衔接；法术成功通知、死亡产物和胜负检查的先后会影响收集或触发。

推荐处理（未采纳）：推荐区分“已不能作为存活单位操作”与“完成事件后的离场／胜负”：后续步骤复查生命资格，原事件已合法结果保留；死亡派生与法术成功通知的相对顺序在涉及它们前明示。最后敌人死亡后不能依赖下一次释放或后续阶段继续收集。
...
触发情境：已错过阶段转下一刻已明确；在当前阶段处理过程中产生的新事件，是本阶段追加还是下一刻处理，并未普遍闭合。循环条件也不能自动成为免费响应。

推荐处理（未采纳）：推荐首批事件型内容先声明观察事件、扣费／计次、入队时点、阶段、同刻顺序和有限响应规则；未声明时不增加额外施法。可先将同阶段新派生事件统一延至下一刻作为候选，避免递归；明确属于当前完整效果的有限子步骤仍在当前事件内完成。

影响与边界：这是拟议默认，不覆盖已有状态周期／到期顺序；复杂传播和镶嵌触发继续后置。
...
触发情境：允许真实元素对象已有方向，但生成位置或锚点、实例身份、数量、结束时点和作用阶段仍未定义，无法判断下一句是否能引用。

推荐处理（未采纳）：随生成物词条填写出现位置的固定规则、身份粒度、来源、主变化量、窗口、结束方式及下一次可引用条件；不开放战中选落点，也不自动采用6刻或一次使用。只需补当前卡会用到的字段。
...
触发情境：先检查资格和三张上限已有规则；“每对象一次”尚未采纳。满额、空产出池或概率未中时是否消耗次数／费用，及同事件多个产出争最后名额仍需词条定义。

推荐处理（未采纳）：产出卡明示次数归属对象／效果／战斗、候选池、概率和空池处理、何时消耗额度与费用、对象变化。推荐先校验资格，再按明确规则尝试与计次；不要默认失败重抽到成功。先用确定产出减少首批依赖；概率机制只在该卡需要时补齐。
...
触发情境：双方扣血和法术禁疗已采纳，但FAT-C独立生命损耗、不打断、护甲不抵消及环境阶段同扣尚未成为正式规则，不能直接与C01二分法混用。

推荐处理（未采纳）：保留整套FAT-C的候选身份并合批审查；需实测时允许明确以候选规则运行，但报告不得称正式基线。若未来采纳独立损耗类别，同步核心、C01、打断与统计口径。禁疗不解除战斗治疗后置范围。
...
触发情境：EX03对同阶段发布战斗很宽，EX04却区分健康入场与残血。玩家主动带残血进危险路线、极端低防配置早败，不应自动等价于常规战斗设计缺陷；“唯一最优”也不能从一个场景判断。

推荐处理（未采纳）：推荐分开常规体验群体、合法极端配置、真实玩家分布；所有配置检查有限终局，秒杀阈值对预先登记的正常入场条件评估，残血／主动风险单独报告并保留警报。合理失败不是静默删样本；速度、损耗与资源的非支配关系跨场景比较。

影响与边界：这是EX候选口径的待议调整，当前没有据此放行任何例外，也不修改既有测试结果。
...
触发情境：现有6卡2杖是测试装置，正式起始资源与范围未定；只给新流派全部核心卡，会把获取与组件机会成本忽略，2.30／1.60成长预算也没有实际资源支撑。

推荐处理（未采纳）：每批固定词池、实际副本、法杖数与允许范围、起始生命、敌人矩阵、获取资格和替换方式；小批先用明确测试装置，结论限于该装置。卡牌可设计，正式成长／路线／经济评价在对应输入齐备后再做，不强行补出全局内容。
...
本轮没有创建具体卡牌、流派GDD或新玩法提案，没有采纳GR推荐方案。下一批测试启动前将卡牌候选、所需全局决定、框架与疲劳包一起固定版本；不同候选规则可以分组测试，但不能混入同一结果集后比较。
...
已采纳规则保持权威来源；GR条目在用户决定或首张相关卡牌设计时逐项补齐资格，按需要晋级素材／提案／评估／Draft Change，并在本页记录关闭依据。测试按[固定交接](test-handoff.md)合批积累，本轮所有新待测项均NotRun。
