...
S2／E3范围现按[RC06](../../../../../../game-design-workflow/idea-materials/M-2026-09-13-card-pool-rule-rulings.md)：原完整事件及胜负检查后，战斗继续则即时处理一层触发；触发结果不再引起其他特殊触发，基础伤害和清理照常。每件特效对同目标同原事件至多一次。独立后续完整复诵与正常状态周期按新主事件处理，复诵不递归复诵。旧GR07-C01同阶段派生延至下一刻保留历史候选，在本范围不再使用。范围外新内容仍须逐条声明，不自动开放复杂传播。
