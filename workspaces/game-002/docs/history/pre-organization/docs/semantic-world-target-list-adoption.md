...
> 原路径：`workspaces/game-002/docs/semantic-world-target-list-adoption.md`。本副本仅调整相对链接，原字节SHA见整理清单。
...
- 已采纳完整法术开始处理时确定一次直接名单；本次不追加、重选或补位，仍逐对象读取当前合法性与材料，新符合条件对象留给后续释放。
- 用户要求一次列出剩余决定；[R01–R32总表](../../../../game-design-workflow/idea-inbox/2026-09-10-semantic-world-remaining-decisions.md)包含推荐、替代、依赖、后置系统与已授权的数值工作。新建议均未采纳。
- 正式引用素材、提案、评估、[采纳文本](../../../../game-design-workflow/draft-changes/D-2026-09-10-single-release-target-list.md)、核心、词汇、决定入口和受影响素材已同步。合格素材43份，inbox记录14份。
- R12位移与R17遗体会改变当前明确边界；其他新建议也必须获确认后进入正式规则。完整召唤、镶嵌和BF1–BF3仍保持既有后置范围。
- 原始版本与采纳时证据继续保留；本次修改前基线为7876e34。此处仅更新现用规则与入口，不把文档修改视为玩家验证结果。

## 本次文件范围

共40份文件。仅提交下列文件；仓库其他删除和临时文件不属于本次改动。
...
已检查本次40份文件：UTF-8严格解码通过，576处相对链接均可解析；合格素材实数与索引均为43，inbox实数与索引均为14；R01–R32连续且无重复。核心、引用素材和SW02-A采纳文本的名单正文完全一致。现用入口已更新为名单已采纳、剩余32项待选择；历史采纳时文本有明确标注。Git补丁空白检查通过。

以上为文档检查；没有执行可玩原型、玩家测试或数值验证，证据保持Hypothesis。
