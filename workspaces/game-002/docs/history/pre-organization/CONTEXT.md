...
> 原路径：`workspaces/game-002/CONTEXT.md`。本副本仅调整相对链接，原字节SHA见整理清单。
...
接口处理范围见[设计决定与后续工作](../../design-decisions-needed.md)，具体参数按[数值任务](../../numerical-redesign.md)重新设计。
...
完整入口：[效果注册与理清](../../effect-registry/README.md)。后续讨论主动登记；未定或Parked案例不能因获得FX编号进入正式实现规格。
...
| 节律杖／节律芯 | 固定十字范围，本杖含简易类型法术的偶数次冷却各减1刻；下限另定。 |
| 余火杖／余火芯 | 固定横三格范围，对范围内合格燃烧新增量加1的法杖身份。 |
| 起始库存 | 每局同一套7词名12实体、4根原木杖、空可换槽及0金币；生命从预设上限开始，上限数值另定。 |
