...
[新版卡池共同规则候选](../game-design-workflow/idea-inbox/2026-09-13-card-pool-rule-recommendations.md)已完成，用户尚未选择。推荐首次来源定特性、同次复诵／邻近合并、即时一层触发及明确支付；双律环明示选敌权限、指令芯跨操控杖与自耗动作许可等局部变更一并列明。当前可直接评议整版，不需先重新回答12组问题；ST01–04保持，候选不改核心、不运行测试。

## 最新：新版55项卡池仍有12组接口待收束

详见[逐卡清晰度审查](card-pool-clarity-audit-2026-09-13.md)。主要是来源合并、回响／复诵、蓄势兑现、结果替换、元素过程、消耗模式与耗尽本体后结果、派生触发和能力归属。纯数值不列为规则歧义，ST01–04已结清。

推荐先处理K04：不同法杖合并同种元素／状态后，来源特效怎样保留和归属；随后处理复诵与支付。现有建议只是Review，不改变E3已确认或S2仍Raw身份，不启动测试。
