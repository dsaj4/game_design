...
- 完整召唤/指挥；法杖起始资源仍待明确。镶嵌转入下方当前系统设计，不预设细则。
- 燃料、自动传播、导电网络和坍塌等复杂环境机制；简单点燃/冰冻已进入当前设计。

## 已处理的接口

C01伤害/生命支付与反馈、C02逐对象数量/成本及整句成功、C06产出资格、C08词卡时间用于冷却继续适用。C03/C04完整召唤及未来单位占位仍后置；C05镶嵌进入当前设计顺序，具体规则待明确。C07保留变化的发生/发展/结果表达，其复杂BF机制后置范围依上表；不再把简单点燃/冰冻一并阻挡。
...
镶嵌下一步补槽位／资格／替换、范围与特殊效果、同种及多枚、触发／计次、获取／收益和跨战／新局边界；详见[系统入口](modifier-and-inlay-design.md)。这些内容尚未成为完整机制，不自动采用此前候选。当前先完成共用系统，再设计流派物品，测试仍暂缓。

## 测试交接

[固定测试交接](test-handoff.md)持续维护。上一批TH-2026-09-11-001已完成限定计算与demo验证，历史报告保留；TH-2026-09-11-002现为r5 Draft积累稿，保留r1／r2固定输入与待测项，追加全局边界、修饰词与后续物品候选；按用户指令暂不运行或分派测试。旧证据复评与新情境分开，合批启动前重新固定输入。执行状态以交接页为准，不重新执行已经通过且输入未变的完整726组，仅在发现差异或扩大范围时追加必要验证。
