...
| 来源ID | 当前情境 | 关系与状态 |
| --- | --- | --- |
| EL-C08 | 有相关释放法术：环境按阈值经历形态后消失变元素；无释放停留末阶段 | FX-065/r2；阈值概念明确，具体判据与身份待定 |
| EL-C09 | 旧元素被抵消消散：有释放则新增对应元素，无释放则不新增 | FX-067/r2、FX-069/r1；补生层数／位置待定 |
| EL-C10 | 草属性单位参与克制，第一阶段没有草状态与草法术 | 阶段范围约束，非新增效果；不阻塞火／冰设计 |

来源：[本轮补充](../../game-design-workflow/idea-inbox/2026-09-12-element-spell-archetype.md)。下方EL-C01–07仍保存原修订关系，不是新版执行证据。

## 元素流派与第一期保留修订

用户保留简易期效果，SC-V06点燃、SC-V07冰冻从活跃卡池删除；下方50项为初始映射，两个动词ID仅供追溯。活跃候选48项。火焰／燃烧及冰霜／冰冻效果仍保留，分别由元素来源承担。

| 来源ID | 当前规则情境 | 效果关系与版本 |
| --- | --- | --- |
| EL-C01 | 有空格且无同种元素，生成元素；已有同种则叠层 | FX-063/r1、FX-010/r3 |
| EL-C02 | 无空格，释放法术向其范围施加状态；出现空格后可再生成 | FX-063/r1、FX-003/r3 |
| EL-C03 | 元素共享状态层数衰减，归零消失；环境转化继承层数 | FX-064/r1、FX-065/r1 |
| EL-C04 | 火对草属性大树与冰对草属性大树，读取固有属性判断增减倍率 | FX-066/r1 |
| EL-C05 | 先修正新施加量，再与旧状态等量抵消；余量建立新状态 | FX-067/r1 |
| EL-C06 | 火焰向附近单位施加燃烧；普通带燃烧的树不自动传播 | FX-068/r1、FX-060/r2 |
| EL-C07 | 释放火焰；火焰伤害敌人；火焰吞噬护甲 | 类型分别简易＋元素、元素、元素＋状态；FX-010/r3、FX-001/r1、FX-023/r1；吞噬具体收益未定 |

来源：[元素素材](../../game-design-workflow/idea-materials/M-2026-09-12-element-spell-archetype.md)。本批情境是待定义规则检查，不是已执行测试。元素在抵消中消散后的余量不能自动赋予不存在宿主。
...
| 来源限定ID | 内容 | 效果关系 | 状态 |
| --- | --- | --- | --- |
| 简易流派/SF01 | 连续完整释放，每次参与共享槽 | [FX-033/r1](entries/FX-033.md) | 方向已确认；实际句子及次数／间隔未定，不能据物品套法术类型 |
| 简易流派/SF02 | 冷却缩减 | 优先评议[FX-026/r1](entries/FX-026.md)，区别[FX-018/r1](entries/FX-018.md) | 没有新增同义ID；本轮尚未选定具体载体或候选细则 |
| 简易流派/SF03 | 本杖成功积累，下次强化伤害或护甲并清空 | [FX-034/r1](entries/FX-034.md) | 方向已确认；门槛、量与失败未定 |

首批锚点建议“伤害敌人／获得护甲／释放火焰”继续引用下方FX-001／002／010原案例与全部类型，未恢复旧起始配置或Parked词效。
...
来源：[本轮合格素材](../../game-design-workflow/idea-materials/M-2026-09-12-first-person-grid-battlefield.md)与[GDD v0.1](../../game-design-workflow/gdd/GDD-2026-09-12-first-person-grid-battlefield.md)。原67项是首轮快照，本节新增关系独立编号，不改历史案例含义。

| 案例 | 句子／过程 | 类型 | 效果版本与状态 |
| --- | --- | --- | --- |
| BF-E01 | 释放火焰，敌人邻近空格生成并持续施加 | 简易＋元素 | FX-010/r2 BF候选→FX-060/r1→FX-003/r2；不是每次脉冲再施法 |
| BF-E02 | 树被持续施加，跨点燃阈值 | 环境结果不是独立法术 | FX-003/r2→FX-061/r1；树与其地面分开 |
| BF-E03 | 草地被持续施加，跨点燃阈值 | 环境结果不是独立法术 | FX-003/r2→FX-061/r1；地面本身为宿主 |
| BF-E04 | 树或草地达到烧焦阈值 | 环境结果不是独立法术 | FX-062/r1；原身份与占位／地面保持，无奖励 |
| BF-E05 | 已燃烧树草旁边的对象 | 无新施法 | 不继续传播，FX-009仍Parked；不会生成新火焰 |
| BF-E06 | 敌人在火焰旁获得燃烧 | “释放火焰”的类型不改变 | FX-060/r1→FX-003/r2单位分支；周期伤害参数未定 |
| BF-E07 | 生成法术满位／取消／结束 | 按原句类型 | FX-010/r2；BF-C04／06／13，不挤占、不复活引用 |
| BF-E08 | 借火焰触发范围燃烧强化 | 镶嵌本身不套类型 | FX-027/r1可选依赖；必须固定IN-C与BF候选版本 |

直接“点燃树木”为简易类；火焰的持续邻近作用不能自动提供“燃烧蔓延”词卡权限。旧TC05及SC-N04等引用r1的地方保留原意，本节明确选择的是BF候选分支。测试边界BF-T01–12进入TH-002/r9，NotRun。

## 单位阈值与替换余量补充（当前）

此前EL-C08／09保留各自修订快照，当前补充如下。

| 案例 | 描述 | 登记与状态 |
| --- | --- | --- |
| EL-C11 | 转化阈值随单位事先设置；环境转化仍继承对应状态层数 | FX-065/r3；阶段衡量判据待定，NotRun |
| EL-C12 | 新入量先修正再抵消；不足保留旧元素，恰好抵消不留新元素，正余量且有相关释放时以余量生成新元素；无释放不新增 | FX-069/r2；位置与同种协调待定，NotRun |

## 火冰推荐覆盖（Raw／未选择）

| 来源案例 | 效果与修订 | 全部类型／边界 |
| --- | --- | --- |
| EL-R01 燃烧结果 | FX-003/r4 | 状态规格；根据完整法术另行分类 |
| EL-R02 冰冻结果 | FX-004/r2 | 状态规格；根据完整法术另行分类 |
| EL-R03 火焰／冰霜吞噬敌人的护甲 | FX-023/r2 | 元素＋状态；对象配对与数量未冻结 |
| EL-R04 火焰爆裂敌人 | FX-070/r1 | 元素；语义例句，句法切分与范围待补 |
| EL-R05 冰霜凝结我的护甲 | FX-071/r1 | 元素＋状态；语义例句，句法切分与费用待补 |
| EL-R释放／直接伤害例句 | FX-063/r1、FX-001现行框架 | 省略主语的释放句为简易＋元素；元素执行者伤害句为元素，不自动附加状态 |

## 2026-09-13当前修订

| 来源案例 | 当前登记 | 变化 |
| --- | --- | --- |
| 第六项冰冻效果 | FX-004/r3 | 冰冻增加护甲，与燃烧伤害对应；EL-R02减速为已替代历史推荐 |
| 火焰／冰霜消耗敌人的护甲 | FX-023/r3 | 元素＋状态；只改名，实际消耗与元素转层数效果不变 |

前述EL-R02／03表保留当时版本，不作为当前命名和冰冻效果。护甲获得时点／层数关系等待补，未运行测试。

## 元素卡池48项覆盖（2026-09-13，Raw）

来源：[元素流派创意卡池](../../game-design-workflow/idea-inbox/2026-09-13-element-spell-creative-card-pool.md)。对象引用不人为创造效果ID，复用旧效果不恢复旧参数；★仅初筛建议。

| 卡ID与名称 | 效果映射 | 状态 |
| --- | --- | --- |
| EC-N01 火焰 | FX-010／FX-063／FX-064／FX-068 | Raw／词义适配待补 |
| EC-N02 冰霜 | FX-012／FX-063／FX-064／FX-068 | Raw／词义适配待补 |
| EC-N03 燃烧 | FX-003／FX-038／FX-005 | Raw／词义适配待补 |
| EC-N04 冰冻 | FX-004／FX-006 | Raw／词义适配待补 |
| EC-N05 护甲 | FX-002／FX-023 | Raw／词义适配待补 |
| EC-N06 敌人 | 对象引用 | Raw／词义适配待补 |
| EC-N07 自己 | 对象引用 | Raw／词义适配待补 |
| EC-N08 树木 | FX-065／FX-066 | Raw／词义适配待补 |
| EC-V01 释放 | FX-063 | Raw／词义适配待补 |
| EC-V02 伤害 | FX-001 | Raw／词义适配待补 |
| EC-V03 消耗 | FX-023 | Raw／词义适配待补 |
| EC-V04 强化 | FX-013／FX-007 | Raw／词义适配待补 |
| EC-V05 削弱 | FX-014／FX-007／FX-064 | Raw／词义适配待补 |
| EC-V06 爆裂 | FX-070 | Raw／词义适配待补 |
| EC-V07 凝结 | FX-071 | Raw／词义适配待补 |
| EC-V08 引爆 | FX-038 | Raw／词义适配待补 |
| EC-V09 熄灭 | FX-005 | Raw／词义适配待补 |
| EC-V10 解除 | FX-006 | Raw／词义适配待补 |
| EC-A01 炽烈的 | FX-072 | Raw／词义适配待补 |
| EC-A02 温润的 | FX-073 | Raw／词义适配待补 |
| EC-A03 绵长的 | FX-074 | Raw／词义适配待补 |
| EC-A04 短命的 | FX-075 | Raw／词义适配待补 |
| EC-A05 浓缩的 | FX-076 | Raw／词义适配待补 |
| EC-A06 弥散的 | FX-077 | Raw／词义适配待补 |
| EC-A07 贪食的 | FX-078 | Raw／词义适配待补 |
| EC-A08 护生的 | FX-079 | Raw／词义适配待补 |
| EC-A09 避亲的 | FX-080 | Raw／词义适配待补 |
| EC-A10 孤立的 | FX-081 | Raw／词义适配待补 |
| EC-A11 簇拥的 | FX-082 | Raw／词义适配待补 |
| EC-A12 易燃的 | FX-046 | Raw／词义适配待补 |
| EC-A13 轻巧的 | FX-039 | Raw／词义适配待补 |
| EC-A14 易塑的 | FX-083 | Raw／词义适配待补 |
| EC-I01 引火芯 | FX-027 | Raw／词义适配待补 |
| EC-I02 聚霜芯 | FX-084 | Raw／词义适配待补 |
| EC-I03 余烬环 | FX-085 | Raw／词义适配待补 |
| EC-I04 融霜环 | FX-085 | Raw／词义适配待补 |
| EC-I05 满格石 | FX-086 | Raw／词义适配待补 |
| EC-I06 续存钉 | FX-087 | Raw／词义适配待补 |
| EC-I07 换相晶 | FX-088 | Raw／词义适配待补 |
| EC-I08 焦土印 | FX-089 | Raw／词义适配待补 |
| EC-I09 霜壳扣 | FX-090 | Raw／词义适配待补 |
| EC-I10 蚀甲齿 | FX-091 | Raw／词义适配待补 |
| EC-I11 燃纹镜 | FX-054 | Raw／词义适配待补 |
| EC-I12 冻甲镜 | FX-092 | Raw／词义适配待补 |
| EC-I13 耗能环 | FX-093 | Raw／词义适配待补 |
| EC-I14 归霜扣 | FX-094 | Raw／词义适配待补 |
| EC-I15 清场晶 | FX-095 | Raw／词义适配待补 |
| EC-I16 轮替座 | FX-096 | Raw／词义适配待补 |

代表句全部类型：释放护生的冰霜／释放避亲的火焰为简易＋元素；火焰消耗敌人的护甲、冰霜凝结自己的护甲为元素＋状态；火焰爆裂敌人为元素；引爆敌人的燃烧为简易＋状态。句法切分与效果规格未闭合，不因例句存在就视为可执行。
