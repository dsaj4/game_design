...
日期：2026-09-05
Project ID：game-002
状态：整页 Raw Idea / Unqualified；时间补牌与多张预览已独立晋级；牌序干预仅预留，具体规则待确认
...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。A 路线、时间补牌、多张牌序显示及干预机制预留均有用户来源；具体抽取顺序、干预规则、打断弃牌和起手保障仍待确认。基本供给与信息机制已清楚，可独立晋级，不把扩展机制一起晋级。
...
- “删牌魔法”是把目标词卡移出本场战斗循环，还是从本局卡组永久删除？推荐基础战斗版本先按本场移除；该范围由用户决定。
...
用户选择显示多张即将补入的牌序，以增加决策信息；具体张数后置，不将“多张”自动解释为“整副牌库完全公开”。用户同时要求预留联想、回忆、改序魔法和删牌魔法，尚未确定这些能力的具体规则，也没有授权实现。

当前推荐框架：时间补牌负责供给节奏；多张预览提供未来信息；卡组构成决定可抽取的组件；牌序干预作为需要获得的能力改变组件先后。洗牌后依次抽取仍是 agent 候选，并未因为多张可见而自动确定初始牌序。

### 预览区的职责建议

- 显示抽牌堆顶部连续若干张词卡及先后，建议同时显示依当前节拍推算的到达时点。具体张数、满手阻挡、洗回后的未知信息后续定。
...
以下是 agent 为避免职责重叠提出的分工，用户当前仅要求预留这些方向，不能将表格视作已采纳的卡牌规则。
...
| 删牌魔法 | 将选定词卡移出某个循环范围 | “本场移除”与“本局永久删除”影响不同，当前必须保留范围未定，不等同普通弃牌 |

删牌范围比较：普通弃牌仍可洗回，不是永久删牌；本场移除影响当前战斗的循环池，下场恢复；本局永久删除改变后续每一场的卡组。推荐先按本场移除设计基础战斗能力，永久删牌另行评议，避免可重复施法无条件压缩整局卡组。跨局收藏删除没有用户依据，不在讨论范围内。
...
当前只预留职责、来源区域、时间衔接和待定边界；不展开卡牌数值，不启动实现。下一问聚焦删牌作用范围，随后回到初始牌序、满手及词卡去向等整体循环规则。
...
满手行为会直接影响等待价值，不能忽略。可比较暂停补牌进度、溢出弃牌、或满手跳过本次补牌。当前推荐候选：保持固定节拍，满手时不抽顶牌、该次补牌机会不累计，顶牌留待后续有空间的节点；预计到达时间应显示受容量阻挡。这样不会因为挂满手等待而储存一串瞬时补牌，也不会自动丢掉已预览的词。此规则尚未确认。
...
- [ ] 先确认删牌魔法的作用范围，再回到排序与满手处理；不继续追问旧版单张预览或补满建议。
