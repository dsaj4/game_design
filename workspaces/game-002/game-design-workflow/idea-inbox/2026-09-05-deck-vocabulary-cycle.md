...
日期：2026-09-05
Project ID：game-002
状态：整页 Raw Idea / Unqualified；时间补牌、多张预览及基础牌序已进入合格素材；基础删牌的本场移除范围已确认，其余循环与干预规则待确认
...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。A 路线、时间补牌、多张显示、基础洗牌与顺序抽取及本场移除范围已有用户依据；干预规则、打断弃牌、满手与起手保障仍待确认。基础牌序已同步供给与信息素材，不把扩展机制一起晋级。
...
- 满手时是否跳过本次补牌、不消耗顶牌且不累计补牌额度，腾出空位后等下一固定补牌节点？推荐采用，既保留预览中的词，又避免囤积补牌机会；具体手牌容量与弃词规则后置。
...
删牌的本场作用范围及基础牌序已确认，其余扩展仍只预留职责、来源区域、时间衔接和待定边界；不展开卡牌数值，不启动实现。下一问处理满手时的补牌，再处理词卡去向与腾位途径。
...
- [ ] 确认满手时的补牌处理，再复核腾位与词卡去向；不重复追问已确认规则。
