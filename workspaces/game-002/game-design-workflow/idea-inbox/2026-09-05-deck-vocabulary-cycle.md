...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。A 路线、时间补牌、多张显示、基础牌序、本场移除范围及满手正常补入后超限弃牌已有用户依据；干预规则、打断弃牌、超限选择的时间衔接与起手保障仍待确认。明确的供给与信息规则已同步素材，不把扩展机制一起晋级。
...
- 超限弃牌时是否暂停战斗时间，只允许选择弃牌，完成后继续原行动？推荐采用；如果当时正在施法，不因弃牌改句、取消或开启第二次施法。此衔接尚待确认。
...
“马上”在此表示超限必须立即处理，尚未确定现实操作是否限时。建议超限时暂停战斗时间，玩家只完成弃牌，随后继续原行动；若当时正在施法，不可利用此窗口改句、取消或另起施法。参与当前法术的词是否处于独立施法区仍属前序候选，不能把它们默认算作可弃手牌。超限选择与敌方行动同一时点的先后也留待明确。

本轮只确认满手仍补、超限弃至上限的机制，未将上述暂停与权限建议一并采纳。验证重点：旧词能否被替换、新词是否可舍弃、施法中是否频繁弹出弃牌操作，以及留牌是否收敛到固定套组；本轮未做试玩或实现。
...
删牌的本场作用范围、基础牌序及满手补入后超限弃牌已确认，其余扩展仍只预留职责、来源区域、时间衔接和待定边界；不展开数值，不启动实现。下一问处理超限弃牌的时间暂停与权限，再回到施法投入词的去向。
...
- [ ] 确认超限弃牌时是否暂停且只开放弃牌，随后复核施法投入词去向；不重复追问已确认规则。
