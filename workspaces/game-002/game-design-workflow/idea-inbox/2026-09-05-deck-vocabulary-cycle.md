...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；普通词卡奖励允许全部跳过、同名独立副本与三张上限均已确认并扩充入组素材；临时复制、删除仅限本场已确认，临时副本可使本场同名数量超过三张也已确认，副本正常弃置与洗回也已确认，具体复制设计已按用户要求后置至相关卡牌出现；敌方行动后接续下一次准备及连续计时已确认，意图的确定与变更也已确认，当前澄清战场状态的跨行动保留；等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
当前工作：[敌方行动后的时间轴延续](2026-09-05-yanzhou-core-combat.md#敌方行动后时间轴延续确认记录)已确认并扩充素材，[敌方意图的确定与变更](2026-09-05-yanzhou-core-combat.md#敌方意图确定与变更确认记录)也已确认；当前澄清[战场状态的跨行动保留](2026-09-05-yanzhou-core-combat.md#当前整体机制评议战场状态的跨行动保留)。
...
- [ ] 当前澄清战场状态的跨行动保留，继续推进状态再利用与供给的机制关系。
