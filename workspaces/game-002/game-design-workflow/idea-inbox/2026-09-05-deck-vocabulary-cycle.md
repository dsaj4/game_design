...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；普通词卡奖励允许全部跳过、同名独立副本与三张上限均已确认并扩充入组素材；临时复制、删除仅限本场已确认，临时副本可使本场同名数量超过三张也已确认，副本正常弃置与洗回也已确认，具体复制设计已按用户要求后置至相关卡牌出现；敌方行动后接续下一次准备及连续计时已确认，意图的确定与变更也已确认，战场状态跨行动保留也已确认并独立晋级，引用不自带消耗、提前施法权限及必需材料缺失落空处理均已确认并同步素材；基础版不提供通用主动取消也已确认；普通战斗胜负目标已独立确认，战败结束本局的后果也已确认并扩充[胜负素材](../idea-materials/M-2026-09-05-normal-combat-outcomes.md)；普通胜利后的生命保留已确认并独立晋级；临时战场状态默认战后清除已确认并扩充状态素材；终点首领战胜利即通关并结束本局已确认；途中休整的恢复与构筑互斥已确认并独立晋级；休整随机卡包三张候选词卡选一张已确认，升级暂缓；先选分支再开包、可拒收及满额过滤已确认；同包三张候选互不重名已确认；休整卡包主谓宾各一张、各类内部随机已确认；选择收益前检查完整卡包、某类无合格词时不提供拿牌并保留恢复已确认；休整一次有限回血、最多补至生命上限已确认，轻伤可补满；2026-09-06 下一次休整的位置提前可知已确认；局内分叉路线已确认并独立晋级；选路前的下一步活动类型与战斗主要信息预告已确认；路线单向推进已确认并扩充素材；每局开局按约束随机安排、局内稳定已确认；预设起始卡组与上局构筑不带入已确认并独立晋级；普通战斗词卡奖励已独立确认，每次普通胜利一次奖励机会已确认，多敌人独立行动已确认，击败后取消行动已确认，完整行动后检查已确认，普通战斗同时满足条件时失败优先已确认，普通敌人生命耗尽即击败已确认，伤害、护甲与打断 D1-D4 已确认，单份状态计时与到期 S1-S5 已确认并独立晋级，同种状态重施与叠加 R1-R5 已确认并独立晋级，状态读值与同刻内部顺序 T1-T5 已确认并独立晋级，普通敌人被击败后的对象与状态 H1-H4 已确认并独立晋级，基础句对象选择与锁定 G1-G3 已确认并独立晋级，多敌人同刻行动顺序 E1-E3 已确认并扩充时间素材，施法预算与未知后续行动 B1-B3 已确认并扩充时间素材，战斗开局与初始节拍 O1-O4 已确认并扩充时间与供给素材，旧 L1-L3 保留历史，整句组合与词义复用已由 SM1-SM3 在新句式下独立确认；防护身份已由 PA1-PA3 确认，ER1-ER3 已明确施法效果来源与修正，PS1-PS3 已于 2026-09-07 明确周期状态强化继承，GI1-GI3 已明确全局道具基础持有与生效，奖励方式已由 GR 独立确认，持有原则沿 MQ，EC1-EC4 已独立晋级，MS1-MS5 已独立晋级，MQ1-MQ4 已独立晋级，MN1-MN4 已独立晋级，GR1-GR4 已独立晋级，NR1-NR3 已独立晋级，F01-F15 已整包确认，当前进入基础数值与流派定轨；等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
| 构句并投入 | 构句暂停；确认后投入词离手进入不占容量、不可超限弃牌的施法区，开始推进时间，基础版不提供通用主动取消 | 特殊词卡对施法区的干预 |
| 施法结束 | 成功、被打断或因必需材料完全缺失而落空的普通投入词都弃置；结束不额外补牌，已耗时间不返还 | 特殊外部终止、特殊词卡去向与特殊触发顺序 |
...
| 同刻结算 | 法术完成及弃牌 → 敌方行动及打断弃牌 → 到时状态 → 固定补牌与超限弃牌；中间不开新施法；状态阶段见[后续确认](2026-09-05-yanzhou-core-combat.md#单份周期状态计时与到期整组确认记录)；同刻敌人固定序见[E1-E3](2026-09-05-yanzhou-core-combat.md#多敌人同刻行动顺序整组确认记录)，状态内部顺序见[T1-T5](2026-09-05-yanzhou-core-combat.md#状态读值与同刻内部顺序整组确认记录) | 特殊新增敌人、连锁触发与特殊抽牌 |

文档层面的普通流程核对：投入词离手腾出空间，但不立即补牌；施法期间到牌仅计入手牌，超限时也只能弃手牌；暂停不触发结束弃置。完成（包含必需材料缺失落空）或打断产生的弃牌先于同刻固定补牌，补牌窗口不能插入新施法。普通牌区流转与事件顺序已有衔接，主动等待终点及特殊触发仍待定；未进行试玩或完整机制验证。
...
| 获得、移除、升级改变卡组表现 | 历史候选；后续确认休整随机卡包三选一，升级已按用户要求暂缓，永久移除未确认 | 局间选择要能改变局内表达，不能只有词卡数量增长；各项权限以独立确认为准 |
...
已明确的局间构筑包括收下普通新词，以及跳过普通战斗词卡奖励；休整已补充随机卡包三张候选词卡选一张，仍与恢复互斥。拿牌改变以后能形成的句子以及组件出现机会。升级按用户要求暂缓，永久移除未确认；普通词卡弃置不会永久缩小本局卡组。后续已明确先选分支再开包、开包后可全部放弃，候选排除本局同名已满三张的词，放弃不返还恢复机会；来源见[开包与过滤确认](2026-09-05-yanzhou-core-combat.md#休整先选分支开包与候选过滤确认记录)。后续[同包不重名确认](2026-09-05-yanzhou-core-combat.md#同一卡包候选不重名确认记录)已补齐三张候选互不重名，跨卡包仍可出现未满额的已有词；后续[三类各一张确认](2026-09-05-yanzhou-core-combat.md#休整卡包三类各一张确认记录)已固定主谓宾各一张、各类内部随机，仍只能收一张或全部放弃；后续[卡包可用条件确认](2026-09-05-yanzhou-core-combat.md#休整卡包可用条件确认记录)要求选择收益前检查完整卡包，某类无合格词时本次不提供拿牌、保留恢复，不自动回血。局外解锁与持久成长继续保持未定。
...
当前工作：[敌方行动后的时间轴延续](2026-09-05-yanzhou-core-combat.md#敌方行动后时间轴延续确认记录)已确认并扩充素材，[敌方意图的确定与变更](2026-09-05-yanzhou-core-combat.md#敌方意图确定与变更确认记录)也已确认；[战场状态的跨行动保留](2026-09-05-yanzhou-core-combat.md#战场状态跨行动保留确认记录)也已确认并独立晋级；[状态引用与消耗的关系](2026-09-05-yanzhou-core-combat.md#状态引用与消耗关系确认记录)也已确认并扩充素材；[提前施法权限](2026-09-05-yanzhou-core-combat.md#提前施法确认记录)也已确认并同步素材；[必需材料缺失时的落空、弃牌与不退时间](2026-09-05-yanzhou-core-combat.md#必需材料缺失落空确认记录)也已确认，并已同步本页当前循环汇总及供给素材；[基础版不提供通用主动取消](2026-09-05-yanzhou-core-combat.md#基础版不提供通用主动取消确认记录)也已确认并同步素材；[普通战斗胜负目标](2026-09-05-yanzhou-core-combat.md#普通战斗胜负目标确认记录)也已确认并独立晋级；当前澄清[战败是否结束当前一局](2026-09-05-yanzhou-core-combat.md#当前整体机制评议战败与本局结束)，该候选仍为 Raw Idea / Unqualified。
...
- [x] 用户确认敌方行动结束即开始下一次准备，战斗时间与补牌计时连续；确认来源与资格复审见原稿。
- [x] 用户确认敌方意图的确定、公开及准备中变更原则，已扩充时间素材。
- [x] 用户确认战场状态的跨行动保留，已独立晋级状态素材。
- [x] 用户确认引用状态本身不附加消耗，消耗与变化由具体操作决定；已扩充状态与构句素材。
- [x] 用户确认满足其他条件时允许在引用材料未出现前提前施法；已扩充构句素材并同步状态与时间素材。
- [x] 用户确认必需材料完全缺失时基础三词句整句落空、普通投入词弃置、已耗时间不返还，结束不额外补牌；已同步当前循环汇总与供给素材。
- [x] 用户确认基础版开始施法后不提供通用主动取消；已同步当前循环汇总与供给素材。
- [x] 用户确认普通战斗击败全部敌人获胜、玩家生命耗尽则失败，已独立晋级胜负素材。
- [x] 普通战败结束本局已确认并扩充胜负素材；本局卡组的跨战斗持有不等于跨局继承。
- [x] 普通胜利后的生命保留已确认并独立晋级[生命素材](../idea-materials/M-2026-09-05-post-victory-health-persistence.md)。
- [x] 临时战场状态默认战后清除已确认，词卡沿用自己的归属和持续范围。
- [x] 终点首领战胜利即通关并结束本局已确认，本局卡组持有不等于通关后继续推进该局。
- [x] 途中休整中恢复生命与改善卡组互斥已确认并独立晋级。
- [x] 休整随机卡包提供三张候选词卡、选择一张入本局卡组已确认并扩充休整素材；升级暂缓。
- [x] 先选拿牌或恢复再开包、开包后可全部放弃、候选排除本局同名已满三张的词已确认；已扩充休整素材，先看再选建议未采纳。
- [x] 用户确认同一卡包三张候选互不重名，跨卡包仍允许未满额的已有词出现；已扩充休整素材。
- [x] 用户确认基础版休整卡包主语、谓语、宾语各一张，各类合格词卡内随机；已扩充休整素材。
- [x] 用户确认选择收益前检查完整卡包，某类无合格词时不提供拿牌、保留恢复；已扩充休整素材。
- [x] 用户确认休整一次有限回血、最多补至生命上限，轻伤可补满；已扩充休整素材并同步生命保留关系，具体恢复量后置。
- [x] 2026-09-06 用户确认下一次休整的位置提前可知；已扩充休整素材并同步生命保留关系，具体路线与间隔后置。
- [x] 局内分叉路线已确认并独立晋级，正式素材增至 9 份，具体布局后置。
- [x] [选路前遭遇信息](2026-09-05-yanzhou-core-combat.md#选路前遭遇信息确认记录)已确认，扩充分叉路线素材，仍为 9 份。
- [x] [路线单向推进](2026-09-05-yanzhou-core-combat.md#路线单向推进确认记录)已确认，进入后不可撤回，完成节点不可回访或重复触发，后续分叉仍可选择。
- [x] [每局路线生成](2026-09-05-yanzhou-core-combat.md#每局路线生成确认记录)已确认，开局按约束随机确定路线与遭遇安排，局内保持稳定。
- [x] [新局起始卡组](2026-09-05-yanzhou-core-combat.md#新局起始卡组确认记录)已独立晋级为第 10 份合格素材；预设起点与上局构筑不带入已确认。
- [x] [普通战斗词卡奖励](2026-09-05-yanzhou-core-combat.md#普通战斗词卡奖励确认记录)复用休整卡包规则并作为独立战后收益已确认，独立晋级第 11 份合格素材。
- [x] [普通胜利奖励节奏](2026-09-05-yanzhou-core-combat.md#普通胜利奖励节奏确认记录)已确认：每次普通胜利一次机会，卡包可用时开一包，可拿一张或全部放弃；已扩充奖励素材。
- [x] [多敌人独立行动](2026-09-05-yanzhou-core-combat.md#多敌人独立行动确认记录)已确认：各自准备并公开行动及时间，共用一条时间轴；补牌仍为同一套固定节拍，已扩充时间素材。
- [x] [击败敌人取消行动](2026-09-05-yanzhou-core-combat.md#击败敌人取消行动确认记录)已确认，普通敌人取消待执行行动并停止后续准备；其余补牌与施法沿用既有规则，整场结束后停止处理。
- [x] [完整行动后检查](2026-09-05-yanzhou-core-combat.md#完整行动后检查击败与胜负确认记录)已确认，每句法术或每次敌方行动完整结算后立即检查击败与胜负，结束后不再执行后续补牌。
- [x] [普通战斗失败优先](2026-09-05-yanzhou-core-combat.md#普通战斗同时满足条件时失败优先确认记录)已确认，同一检查点两项条件均成立时按战败结束本局，后续供给停止。
- [x] [普通敌人生命耗尽即击败](2026-09-05-yanzhou-core-combat.md#普通敌人生命耗尽即击败确认记录)已确认，已扩充胜负素材并同步时间关系。
- [x] [伤害、护甲与打断](2026-09-05-yanzhou-core-combat.md#伤害护甲与打断整组确认记录)D1-D4 已整组确认，独立晋级第 12 份素材。
- [x] [单份状态计时与到期](2026-09-05-yanzhou-core-combat.md#单份周期状态计时与到期整组确认记录)S1-S5 已整组确认，独立晋级第 13 份素材；已同步固定补牌前的状态阶段及结束检查。
- [x] 用户[修改 R3](2026-09-05-yanzhou-core-combat.md#r3持续时长相加修改记录)为当前剩余时长加本次新增时长，整组仍留在 inbox，素材数量不变。
- [x] [R1-R5 整组确认](2026-09-05-yanzhou-core-combat.md#同种状态重施与叠加整组确认记录)：R3 沿用时长相加，其余四项确认，独立晋级第 14 份素材。
- [x] [T1-T5 整组确认](2026-09-05-yanzhou-core-combat.md#状态读值与同刻内部顺序整组确认记录)已独立晋级第 15 份素材，固定补牌仍在状态处理之后。
- [x] [H1-H4 整组确认](2026-09-05-yanzhou-core-combat.md#普通敌人被击败后的对象与状态整组确认记录)已独立晋级第 16 份素材；单目标对象被击败后的正常完成落空按既有投入词弃置处理，整场先结束不继续补牌。
- [x] G1-G3 已整组确认，见[基础句对象选择与锁定](2026-09-05-yanzhou-core-combat.md#基础句的对象选择与锁定整组确认记录)，独立晋级第 17 份素材。
...
- [ ] 按[灵活句式、词性复用与多类型改动](2026-09-06-flexible-grammar-and-spell-types.md)重议旧 L1-L3，SG1-SG5、FG1-FG5、ST1-ST6、SL1-SL6、SH1-SH6、SC1-SC6、SN1-SN5 和 SR1-SR6 已分别独立晋级，WC1-WC5 已独立晋级[词性分类与卡包适配](../idea-materials/M-2026-09-06-word-classes-and-reward-packs.md)，OH1-OH5 已独立晋级[开局手牌保障](../idea-materials/M-2026-09-06-opening-hand-guarantees.md)，SM1-SM3 已独立晋级[整句效果与词义复用](../idea-materials/M-2026-09-06-compositional-spells-and-word-meaning.md)，PA1-PA3 已独立晋级[防护状态定义与生成](../idea-materials/M-2026-09-06-armor-identity-generation-and-persistence.md)，ER1-ER3 已独立晋级[施法效果的来源与修正](../idea-materials/M-2026-09-06-spell-effect-sources-and-modifiers.md)，PS1-PS3 已于 2026-09-07 独立晋级[周期状态的强化继承](../idea-materials/M-2026-09-07-periodic-state-modifier-inheritance.md)，GI1-GI3 已独立晋级[全局道具的基础持有与生效](../idea-materials/M-2026-09-07-global-item-ownership-and-passive-effects.md)，[GA1-GA4 全局道具奖励与重复限制](2026-09-06-flexible-grammar-and-spell-types.md#当前小板块全局道具奖励与重复限制)尚未确认，待与商店联动复审；EC1-EC4 已独立晋级，当前按 [首轮全局设计基线](../idea-materials/M-2026-09-07-first-pass-global-design-baseline.md) 进入数值与流派阶段；下限数值仍后续明确，关联卡不增加删牌安全余量已由 SL6 确认。
