...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词流转及起手合法句保障已有用户依据；干预规则、洗回触发、同一时点事件顺序与起手保障方法仍待确认。明确的供给与循环规则已同步素材，不把扩展机制一起晋级。
...
- 补牌时抽牌堆已空，是否将当时的弃牌堆洗回，并在该补牌时点继续抽取、不额外耗时？推荐采用；手牌、施法区和本场移除牌不参与洗回。抽弃均空的恢复及同时事件顺序继续另行明确。
...
| 环节 | 已明确内容 | 仍待明确 |
| --- | --- | --- |
| 形成抽取顺序 | 开战及弃牌洗回随机洗牌，之后按序抽取，明确效果才改序；起手保障自有词可构成一条合法基础句 | 起手保障方法、具体牌数、洗回触发边界 |
...
起手合法句保障已确认，具体方法后置。下一问处理洗回触发：建议补牌时抽牌堆已空，就洗回当时的弃牌堆，在本次补牌时点继续抽取，不另收时间、不改变固定补牌节拍。手牌、施法区与本场移除牌不参与该洗回。该完整触发建议尚未确认；抽弃都空时如何恢复、同一时点先弃后抽的关系仍待明确。
...
- [ ] 确认补牌时空牌库的洗回触发与是否额外耗时；不重复追问已确认规则。
