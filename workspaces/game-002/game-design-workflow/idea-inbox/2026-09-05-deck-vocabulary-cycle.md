...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词流转、起手合法句保障及空抽牌堆的洗回触发已有用户依据。安全下限及移除量最多执行到下限、提前展示实际可移除量也已确认并同步供给素材。跨洗回预览及其他扩展机制仍待确认。
...
- 抽牌堆剩余词不足预览范围时，是否只显示现有确定牌序，其后标为“待洗回”，到实际洗回后才显示新牌序？推荐采用；不为填满预览提前洗牌，不改变已确认的洗回时点。该建议尚未确认。
...
下一个关键问题：抽牌堆剩余词不足预览范围时，是否只展示现有确定牌序，其后标为“待洗回”，等实际洗回后才显示新序列？推荐采用，不为填满预览提前洗牌。

依据：当前洗回发生在补牌需求遇到空抽牌堆时，洗回前弃牌成员仍可能改变；多张预览不能据此提前确定尚未生成的顺序。采用该候选将使洗回前的信息范围暂时缩小，玩家需要在未知下一轮组件时决定是否继续等待。洗回仍在既定补牌时点完成当次抽取，之后展示新抽牌堆顶部的未来牌序；不把已经到手的牌继续当作未来牌。该候选未获确认，不写入合格素材。
...
| 洗回弃牌 | 补牌遇空抽牌堆时洗回当时弃牌，继续本次抽取，不额外耗时；以卡组下限防止双空堆，本场移除同受约束 | 下限参数；同一时点事件顺序、跨洗回预览 |
...
洗回触发与不额外耗时已确认。用户改为设置卡组下限、防止两堆同时为空，双空堆跳过补牌建议未采纳；移除最多执行到下限并提前展示实际可移除量也已确认。当前讨论跨洗回预览，同一时点事件仍待明确；不因卡组有足够张数就宣称语义卡手已解决。
