...
日期：2026-09-05
Project ID：game-002
状态：Raw Idea / Unqualified；A 已选为细化方向，用户最新提出按时间固定速度补牌；预览与牌序待确认

本文以共享 idea-template 保存来源与资格记录，再组织完整的机制草案。用户已授权细化 A，尚未逐项采纳本页新增规则；本页不是正式 GDD、Proposal 或实现任务。具体卡牌数值、手牌容量、卡组规模、时间费用和转化比例继续后置。

当前版本入口：见“本轮修订：按时间补牌”。用户没有采纳旧版的施法结束补满，而是提出时间驱动补牌。下方 A-01 至 A-12 及旧循环图保留为历史候选，不能与新修订叠加执行。
...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。A 路线及按时间固定速度补牌有用户来源；下一张预览、具体牌序、打断弃牌、起手保障等仍是待评议规则，不能用路线选择代替全部规则确认。

| 资格问题 | 当前答案 | 状态 |
| --- | --- | --- |
| 设计对象及 GDD 章节 | 词汇生命周期、战斗资源循环与局内构筑；对应系统规格、组合、经济与成长章节 | Clear |
| 玩家处境及功能 | 在时间窗口内从手牌组织合法法术，调度以后可用的词 | Clear |
| 玩家行为及可见影响 | 用户提出时间流逝带来补牌，玩家选择现在施法或等待更多词；预览范围与排序方式尚未选择 | Clear（动作方向）；Unknown（信息与顺序） |
...
- 是否公开“下一张词卡的完整内容及其预计到达时点”？推荐公开下一张，更远牌保持未知；本问题不自动确认排序、满手和弃牌规则。

### 资格结论

- [x] Unqualified：时间补牌方向已由用户提出，完整词汇循环的信息与顺序仍待选择；不把本轮评议作为正式 Evaluation。
...
- [ ] 先确认下一张完整预览及预计到达时点，再确认排序与满手处理；不继续追问旧版补满建议。
