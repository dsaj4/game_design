...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。A 路线、时间补牌、多张显示、基础牌序、本场移除范围、超限弃牌及暂停权限、普通投入词结束弃牌已有用户依据；投入词离手及容量占用、干预规则、同一时点事件顺序与起手保障仍待确认。明确的供给与循环规则已同步素材，不把扩展机制一起晋级。
...
- 确认施法时，投入词是否立即离开手牌、进入临时施法区，不占手牌上限且不能用于超限弃牌？推荐采用，结束后按已确认规则进入弃牌堆，避免把正在施法的词与可调度手牌混在一起。
...
| 构句并投入 | 构句暂停，确认后推进施法时间 | 投入词是否立即离手、不占容量且独立存放 |
| 施法结束 | 成功或被打断的普通投入词都弃置；结束不额外补牌 | 特殊词卡去向与具体事件先后 |
| 本场移除 | 被移除词退出本场循环，下场恢复参与 | 具体魔法目标与效果 |

本轮完成文档层面的循环核对：超限弃牌的暂停不会触发结束弃牌；弃牌不会产生额外补牌；打断不回退时间；本场移除不因洗回恢复。涉及投入词容量占用的场景仍无法唯一推演，先作为下一问，不能将整个循环宣称为已验证。
...
普通投入词在施法成功或被打断后统一弃置已确认，不额外附送补牌。下一问明确投入词是否确认后立即离手、进入不占手牌上限的临时施法区；这是候选建议，尚未采纳。之后再复核起手与洗回等环节，不展开数值或启动实现。
...
- [ ] 确认投入词的离手时点及容量占用，再复核起手与洗回；不重复追问已确认规则。
