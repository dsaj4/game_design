...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；普通词卡奖励允许全部跳过也已确认并扩充入组素材，当前评议同名词卡副本；等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
状态：Raw Idea / Unqualified；沿用 A-01 的历史候选进行完善，尚未获用户确认，不进入合格素材的规范化规则。
...
下一个关键问题：是否允许本局卡组保留同名普通词卡的多个独立副本，再次获得并收下时增加一张卡？推荐允许。
...
- [ ] 评议同名普通词卡是否可以持有多个独立副本，数量上限与具体卡牌规则后置。
