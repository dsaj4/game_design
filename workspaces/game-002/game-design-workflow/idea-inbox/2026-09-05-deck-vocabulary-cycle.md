...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词流转、起手合法句保障及空抽牌堆的洗回触发已有用户依据。用户要求以卡组下限防止两堆同时为空，并已确认本场移除也受安全下限约束；该约束已复审并同步供给素材。触及下限的处理及扩展机制不随之晋级。
...
- 移除效果是否最多执行到安全下限，超出可移除余量的部分不执行？推荐采用数量截断，并提前显示实际可移除量；这是待确认候选，不自动决定其他关联效果。原“两堆空则跳过补牌”建议未获采纳。
...
状态更新：本场移除受安全下限约束已由用户确认，下列资格字段已复审，约束同步至[供给素材](../idea-materials/M-2026-09-05-timeline-draw-preview.md)，为 Qualified GDD Material / Hypothesis；具体触限处理仍留在 inbox。

| 资格问题 | 当前答案 | 状态 |
| --- | --- | --- |
| 来源与设计对象 | 本轮用户原话；卡组构筑下限与战斗词汇供给 | Clear |
| 玩家处境及目标 | 持续施法、补牌与移除词卡时，避免抽牌堆和弃牌堆同时无牌 | Clear |
| 玩家行为与约束 | 设置卡组下限，本场移除也不得把仍参与循环的牌数降到安全下限以下；具体数值与触限处理后置 | Clear（约束范围） |
| 预期价值 | 避免无牌可补造成供给中断，维持时间补牌循环 | Clear（目标，未验证） |
| 与现有系统关系 | 下限覆盖本场移除造成的循环池缩小；需要考虑手牌、施法区与超限占用，以限制极端压缩换取持续供给 | Clear（依赖与取舍） |
| 主要未知与验证 | 明确触限处理，再用最大留牌、施法占用、连续补牌及超限窗口复核安全余量 | Clear（方法，未执行） |
...
下一个关键问题：移除效果是否最多执行到安全下限，超出可移除余量的部分不执行？agent 推荐数量截断，并在构句预览显示实际可移除量，让供给保护对玩家可见。若采用，具体目标选择、关联收益是否依实际移除量计算及施法期间状态变化仍需后续明确；不将这些能力细节视为本次已经确认。
...
| 洗回弃牌 | 补牌遇空抽牌堆时洗回当时弃牌，继续本次抽取，不额外耗时；以卡组下限防止双空堆，本场移除同受约束 | 下限参数与触限处理；同一时点事件顺序、跨洗回预览 |
...
| 本场移除 | 被移除词退出本场循环，下场恢复参与；不得将仍参与循环的牌数降到安全下限以下 | 触限处理、具体魔法目标与效果 |
...
洗回触发与不额外耗时已确认。用户改为设置卡组下限、防止两堆同时为空，双空堆跳过补牌建议未采纳；本场移除同受下限约束也已确认。当前讨论触限时的处理，见“卡组下限方向与资格追问”。同一时点事件及跨洗回预览仍待明确；不因卡组有足够张数就宣称语义卡手已解决。
