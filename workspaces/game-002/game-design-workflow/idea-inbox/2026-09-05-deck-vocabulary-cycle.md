...
日期：2026-09-05
Project ID：game-002
状态：整页 Raw Idea / Unqualified；时间补牌与多张预览已独立晋级；删牌基础版本的本场移除范围已确认，其余干预规则待确认
...
- 基础抽取是否采用“开战建立牌库和弃牌洗回时随机洗牌，随后按序抽取，只有明确的效果才改变顺序”？推荐采用，保留构筑分布与多张预览的可信度；不包含具体改序能力和起手保障。
...
删牌的本场作用范围已确认，其余扩展仍只预留职责、来源区域、时间衔接和待定边界；不展开卡牌数值，不启动实现。下一问回到基础抽取顺序，随后处理满手及词卡去向等整体循环规则。
...
- [ ] 确认基础洗牌与顺序抽取规则，再处理满手；不继续追问已确认的删牌范围或旧版单张预览、补满建议。
