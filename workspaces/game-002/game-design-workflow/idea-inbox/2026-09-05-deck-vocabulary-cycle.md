...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；当前澄清普通词卡奖励能否全部跳过，等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
状态：Raw Idea / Unqualified，agent 推荐候选，尚未获用户确认；不进入合格素材的规范化规则。
...
下一个关键问题：普通战斗后的词卡奖励，是否允许玩家一张都不拿、直接跳过？推荐允许。
...
- [ ] 澄清普通战斗后词卡奖励能否全部跳过，具体数量、补偿及其他获取规则继续待定。
