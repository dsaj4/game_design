...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词结束弃置及临时施法区已有用户依据；干预规则、同一时点事件顺序与起手保障仍待确认。明确的供给与循环规则已同步素材，不把扩展机制一起晋级。
...
- 是否为开局手牌提供至少一条合法基础句的保障？推荐仅保障起手可构句，词来自自己的卡组，不保证最优组合或每次补牌都能成句。若采用，应作为随机起手的明确例外，具体实现方式与牌数后置。
...
| 环节 | 已明确内容 | 仍待明确 |
| --- | --- | --- |
| 形成抽取顺序 | 开战及弃牌洗回随机洗牌，之后按序抽取，明确效果才改序 | 起手组成与保障、洗回具体触发边界 |
...
普通投入词的离手、存放和结束去向已确认，下一问处理起手可表达性。推荐起手至少有一条由自有词卡组成的合法基础句，既检查语法类别也检查语义兼容；不保障最优效果或后续每次补牌都成句。此保障仍为候选，若采用需明确为随机起手的例外，具体方法后续设计；不把保障建议视为当前随机洗牌规则已经改变。
...
- [ ] 确认起手是否提供合法基础句保障，再复核洗回；不重复追问已确认规则。
