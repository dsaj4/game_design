...
状态：Raw Idea / Unqualified；以下为 agent 针对基础复制的生成位置建议，尚待用户确认，不写入合格素材。
...
下一个关键问题：是否让基础复制生成的副本默认置于抽牌堆顶部，按原补牌节拍进入手牌？推荐采用。
