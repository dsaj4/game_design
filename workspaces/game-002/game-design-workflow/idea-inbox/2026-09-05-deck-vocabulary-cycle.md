...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；普通词卡奖励允许全部跳过、同名独立副本与三张上限均已确认并扩充入组素材；用户新增允许临时复制、删除，当前澄清其持续范围；等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
状态：独立副本与同名三张上限已通过 grill-with-docs 资格复审，扩充至[新词入组素材](../idea-materials/M-2026-09-05-new-word-deck-inclusion.md)，Qualified GDD Material / Hypothesis；临时复制、删除的新增表达单独保留为 Raw Idea / Unqualified，见下一节。
...
状态：Raw Idea / Unqualified；用户允许临时复制、删除的方向已记录，具体持续范围与复制规则尚未确定；不能将其写成可执行的正式能力。
...
下一个关键问题：这里的“局内临时”是否限定为当前战斗，复制出的卡战后消失、被临时移除的原有卡下一场恢复？推荐采用。
...
- [ ] 澄清“局内临时”是否仅持续本场；复制突破三张的权限及卡牌去向另行复核。
