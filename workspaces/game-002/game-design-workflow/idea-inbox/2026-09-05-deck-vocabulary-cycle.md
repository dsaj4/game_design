...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词流转、起手合法句保障及空抽牌堆的洗回触发已有用户依据。用户最新要求以卡组下限防止两堆同时为空；该方向的战斗内约束尚待澄清。明确的供给与循环规则已同步素材，不把新下限方案和扩展机制一起晋级。
...
- 本场移除是否也受安全下限约束，不能把仍参与本场循环的词卡总数降到下限以下？推荐采用；下限须考虑手牌和施法区的最大占用，具体张数后置。原“两堆空则跳过补牌”建议未获采纳，不继续作为当前推荐。
...
超限选择的时间衔接已由后续确认明确：暂停战斗时间，玩家只完成弃牌，随后继续原行动；若当时正在施法，不可利用此窗口改句、取消或另起施法。投入词在独立施法区，不属于可弃手牌，见后续临时施法区确认。超限选择与敌方行动同一时点的先后仍待明确。
...
状态：Raw Idea / Unqualified。本节保存新方向与机制推演，不将 agent 补充的约束写入合格素材。

| 资格问题 | 当前答案 | 状态 |
| --- | --- | --- |
| 来源与设计对象 | 本轮用户原话；卡组构筑下限与战斗词汇供给 | Clear |
| 玩家处境及目标 | 持续施法、补牌与移除词卡时，避免抽牌堆和弃牌堆同时无牌 | Clear |
| 玩家行为与约束 | 要设置卡组下限；是否同时限制本场移除、如何计算安全下限未明 | Unknown（约束范围） |
| 预期价值 | 避免无牌可补造成供给中断，维持时间补牌循环 | Clear（目标，未验证） |
| 与现有系统关系 | 手牌与施法区都在牌堆外，本场移除会缩小仍参与循环的牌数；仅检查开战卡组张数不足以长期保证目标 | Clear（依赖）；Unknown（取舍） |
| 主要未知与验证 | 先决定本场移除是否受下限限制，再检查最大留牌、施法占用、连续补牌及超限弃牌窗口 | Clear（方法，未执行） |
...
推荐候选：卡组构筑设置下限，本场移除也不得把参与循环的牌数降到安全下限以下；安全下限根据允许的最大手牌与施法区占用留出牌堆余量。具体数字后置。该约束会限制极端压缩卡组，是维持持续供给的代价；不自动恢复已移除牌，不擅自禁用普通补牌或强制弃掉玩家保留的词。
...
下一个关键问题：是否让本场移除也受安全下限约束？具体拦截方式、临时超限窗口及参数在约束方向确认后再细化。
...
| 洗回弃牌 | 补牌遇空抽牌堆时洗回当时弃牌，继续本次抽取，不额外耗时；其他牌区不参与 | 用户要求以卡组下限防止双空堆，约束方式待定；同一时点事件顺序、跨洗回预览 |
...
| 本场移除 | 被移除词退出本场循环，下场恢复参与 | 具体魔法目标与效果 |

本轮完成文档层面的普通流程核对：投入词离手腾出空间，但不立即补牌；施法期间到牌仅计入手牌，超限时也只能弃手牌；暂停不触发结束弃置，原法术继续后才在成功或打断时弃置投入词。普通流程的卡牌位置已能衔接。同一时点的补牌、弃牌窗口与敌方行动顺序仍未定，未进行试玩或完整机制验证。
...
洗回触发与不额外耗时已确认。前轮提出两堆均空则跳过当次补牌、不积攒欠牌；用户改为设置卡组下限、防止两堆同时为空，该跳过建议未采纳。当前先明确本场移除是否也受安全下限约束，见“卡组下限方向与资格追问”。同一时点事件及跨洗回预览仍待明确；不因卡组有足够张数就宣称语义卡手已解决。
...
- [ ] 确认本场移除是否也受安全下限约束，再核对各牌区最大占用；具体数值后置。
