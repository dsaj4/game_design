...
已使用 grill-with-docs 核对原稿、时间素材及 A/B 比较。供给、预览、基础牌序、本场移除范围、超限弃牌暂停、投入词流转、起手合法句保障及空抽牌堆的洗回触发已有用户依据；干预规则、两堆均空的补牌恢复、同一时点事件顺序与起手保障方法仍待确认。明确的供给与循环规则已同步素材，不把扩展机制一起晋级。
...
- 抽牌堆和弃牌堆都空时，是否跳过当次补牌、不累积欠牌，后续仍按原节拍检查？推荐采用；后来出现弃牌不立即补发，等下一次补牌时点再依已确认规则洗回。该建议尚未确认。
...
| 洗回弃牌 | 补牌遇空抽牌堆时洗回当时弃牌，继续本次抽取，不额外耗时；其他牌区不参与 | 两堆均空的补牌恢复、同一时点事件顺序、跨洗回预览 |
...
洗回触发与不额外耗时已确认。下一问处理两堆均空：建议当次补牌落空、不累积欠牌，补牌节拍继续；后来出现弃牌不立即补发，等下一次既定补牌时点再检查并洗回。该建议尚未确认。同一时点事件及跨洗回预览仍待明确。若所有剩余词都被留在手中且不能构句，仅靠等待不能恢复供给，主动整理或其他防卡死方案仍须另行评议，不借本次确认自动加入。
...
- [ ] 确认两堆均空时是否跳过当次补牌且不积累欠牌；不重复追问已确认规则。
