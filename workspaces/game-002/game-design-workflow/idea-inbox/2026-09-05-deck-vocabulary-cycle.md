...
- 当前转入[原稿的构句机制评议](2026-09-05-yanzhou-core-combat.md#当前构句机制评议词卡与句法职责)：用户已明确三类不能混用、两层兼容及以已有状态为宾语仍需投入对应词卡，制造状态本身不自动获得词卡。词卡耗时相加已确认并同步时间素材，基础版独立字数上限已确认暂不启用。新词直接入本局卡组已确认并单独晋级；普通词卡奖励允许全部跳过、同名独立副本与三张上限均已确认并扩充入组素材；临时复制、删除仅限本场已确认，临时副本可使本场同名数量超过三张也已确认，副本正常弃置与洗回也已确认，具体复制设计已按用户要求后置至相关卡牌出现；敌方行动后接续下一次准备及连续计时已确认，意图的确定与变更也已确认，战场状态跨行动保留也已确认并独立晋级，引用不自带消耗、提前施法权限及必需材料缺失落空处理均已确认并同步素材；当前澄清普通施法中的主动取消权限；等待操作与停点后置；循环的缺类、不兼容和状态可用时机错开的风险须结合构句与时间规则复核。
...
| 施法结束 | 成功、被打断或因必需材料完全缺失而落空的普通投入词都弃置；结束不额外补牌，已耗时间不返还 | 主动取消权限、特殊词卡去向与特殊触发顺序 |
...
当前工作：[敌方行动后的时间轴延续](2026-09-05-yanzhou-core-combat.md#敌方行动后时间轴延续确认记录)已确认并扩充素材，[敌方意图的确定与变更](2026-09-05-yanzhou-core-combat.md#敌方意图确定与变更确认记录)也已确认；[战场状态的跨行动保留](2026-09-05-yanzhou-core-combat.md#战场状态跨行动保留确认记录)也已确认并独立晋级；[状态引用与消耗的关系](2026-09-05-yanzhou-core-combat.md#状态引用与消耗关系确认记录)也已确认并扩充素材；[提前施法权限](2026-09-05-yanzhou-core-combat.md#提前施法确认记录)也已确认并同步素材；[必需材料缺失时的落空、弃牌与不退时间](2026-09-05-yanzhou-core-combat.md#必需材料缺失落空确认记录)也已确认，并已同步本页当前循环汇总及供给素材；当前澄清[普通施法中的主动取消权限](2026-09-05-yanzhou-core-combat.md#当前整体机制评议施法中的主动取消权限)，该候选仍为 Raw Idea / Unqualified。
...
- [ ] 当前澄清普通施法中的主动取消权限，继续推进状态、时间与词卡循环的关系。
