...
状态：Raw Idea / Unqualified。日期：2026-09-13。当前为右侧地图桌面 VisualDesignDraft / 整体待确认。此前 E1 R2 已完成独立实机美术实验；本轮回到整体效果图与设计文稿，组件单图和新模型实现等待整体确认。技术证据与候选效果图不代表素材晋级或玩法采纳。
...
用户对杂物的选择不等于整体方向采纳。整体图仍待确认，组件单图尚未生成；没有把本轮位图接入 Godot，也未扩展 Blender 模型。猫、书和额外杂物仅定义视觉位置与装饰作用，不擅自定义玩法功能；金币数值、事件结果和具体路线规则未在此决定。原始想法继续留在 inbox，尚未晋级正式素材。
