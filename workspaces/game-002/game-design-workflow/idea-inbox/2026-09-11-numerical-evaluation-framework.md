...
状态：Promoted：四层数值评估框架候选。日期：2026-09-11。Project ID：game-002。具体目标曲线、权重、阈值与终局机制尚未采纳。
...
无期限战斗的终局保障是单独规则缺口，已列入框架NF-D01；候选推荐明确战斗时限与超时结果，尚未改写胜负规则。其他数值阈值继续作为候选，不阻塞框架文档形成。
...
- [ ] 由测试agent独立审查框架，完成可执行部分；终局保障另行决策。
