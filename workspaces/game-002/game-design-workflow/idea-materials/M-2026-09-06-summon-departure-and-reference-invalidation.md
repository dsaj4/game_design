...
任何召唤引用机制须遵守战前固定配置，战中不能重新分配词卡。引用资源的创建与可用时机尚待C03、C04决定。
...
| --- | --- | --- | --- | --- |
| 必需执行者失效与多目标部分结算、专属引用资源生命周期。 | 影响完整设计与效果可解释性 | 使用两个同种单位，令其中一个离场，检查只失效其专属关系、另一单位不被误清理。 | 玩家可预测结果，关键情境有唯一裁定 | 结果依赖未确认规则，或一类选择长期无价值 |
