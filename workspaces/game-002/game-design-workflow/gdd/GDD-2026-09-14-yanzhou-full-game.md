...
| 关联提案／评估 | [首版补齐提案](../idea-proposals/P-2026-09-14-complete-first-release-design.md)／[评估](../evaluations/E-2026-09-14-complete-first-release-design.md) |
| 关联决策 | [CORE-032–036采纳文本](../draft-changes/D-2026-09-14-complete-first-release-design.md)／[决策记录](../decision-log.md) |
| 关联正式素材／inbox审查 | [68份正式素材、47份原始记录逐项审查](yanzhou-rc1/10-source-review.md)；没有未晋级候选直接充当规则 |
| 关联验证 | [本Wiki验收输入与风险](yanzhou-rc1/09-validation-and-risks.md)；新规格NotRun |
| 关联开发进度 | [开发进度索引](../../docs/code-development-index.md)，只建立设计关联，不在GDD记录实现状态 |
...
[完整审查表](yanzhou-rc1/10-source-review.md)逐一列出68份合格素材的Include／Park、对应章节、替代边界与47份inbox的晋级／隔离结果。所有旧创意和原话保留；源码实验、原始视觉参考和管理决定不被当成新增玩法。
...
[UX02完整流程表](yanzhou-rc1/08-journey-ui-and-assets.md)覆盖菜单、全图、法杖袋管理、三步编排、战斗观看、普通结算、休整、商店、首领胜利、失败、设置与规则册，逐项给出动作、确认结果与返回边界。
...
| 系统 | 目的 | 主要输入 | 下游 |
| --- | --- | --- | --- |
| [SYS-001 实体构句与类型](yanzhou-rc1/01-systems.md) | 把库存词义组合成可解释的完整法术 | 本局库存与本场公开对象 | 合法完整法术与不合法原因；未使用卡留库存 |
| [SYS-002 法杖、镶嵌与时间编排](yanzhou-rc1/01-systems.md) | 把空间与首次相位变成明确取舍 | 已持有法杖与可换物，战外可编辑 | 保存配置与名义时间轴，空杖被动不激活 |
| [SYS-003 自动战斗与状态时序](yanzhou-rc1/01-systems.md) | 让固定配置按公开节拍可靠执行 | 锁定本场配置、敌人、环境及随机输入 | 自动结算、记录、普通胜利或失败 |
| [SYS-004 元素与环境](yanzhou-rc1/01-systems.md) | 提供释放、储层、支付、抵消和转化的共同空间 | 2×5地面＋占位；HG能力；单元素状态 | 火冰新态、元素、增甲／损伤、永久本场痕迹 |
| [SYS-005 路线、敌人与有限终局](yanzhou-rc1/01-systems.md) | 让同一构筑面对不同压力并完成一局 | 固定4种路线配对之一，既定19节点与12遭遇 | 胜负及下一节点；首领胜直接终局 |
| [SYS-006 收益、商店与休整成长](yanzhou-rc1/01-systems.md) | 用有限收入与恢复机会驱动构筑调整 | 当前金币、词卡副本、物品唯一额、开放期别 | 本局持久库存、金币与生命；新局重置 |
| [SYS-007 旅程、阅读与保存](yanzhou-rc1/01-systems.md) | 让所有规则有可完成的操作与可理解反馈 | Windows离线简中；1进行中存档；独立设置 | 完整可操作流程、可查来源／原因与锁定重播 |

## 4. 系统规格

[七个系统的4.1–4.10完整规格](yanzhou-rc1/01-systems.md)逐一覆盖目的／承诺、MDA、输入／状态／输出、核心规则、代价、失败／取消、交互、正反例、规则／体验验收及未知依赖。各章节的参数与规则表属于其4.4延伸，不能只按系统简介制作。

## 5. 状态机与事件时序

完整状态机、四阶段、RC01–12、BR01–11、ST01–04及GR费用规则见[战斗与状态](yanzhou-rc1/03-combat-and-status.md)。元素持续释放、覆盖、邻近和环境末转化见[元素章节](yanzhou-rc1/04-elements-and-environment.md)。
...
[PG完整表](yanzhou-rc1/06-parameters-and-economy.md)逐词列τ、动词L、量与支付、所有物品价格／阶段。普通量最终max(0，floor((基础＋固定增减)×已声明乘数))；自然衰减小数累计，新元素入量先修正后抵消，继承／正余量不重新加成。
...
[53实体逐卡目录](yanzhou-rc1/05-card-catalog.md)每张给短卡面、全部流派ID、实际参数／时间／获取、合法角色、正反例、有限事件与效果追踪。[CG](yanzhou-rc1/02-grammar-and-configuration.md)给全部效果组和多来源合并。
...
[RG完整章节](yanzhou-rc1/07-route-and-encounters.md)给19节点的全部连接、12遭遇的敌人与十格环境、7普通敌人＋1首领的生命／甲／攻击时刻，及每条路线的公开信息和成长机会。无需另查聊天补站位。
...
[UX03／06](yanzhou-rc1/08-journey-ui-and-assets.md)规定可跳过提示与U01–08任务：先看到实体和整句，再看冲突与当前冷却，随后理解元素身份／状态与路线资源。提示不改变敌人、卡池或奖励，跳过仍可查规则册；失败后复盘并从统一资源新开局，不继承局外增强。

## 11. UI、呈现与无障碍

[UX02–05](yanzhou-rc1/08-journey-ui-and-assets.md)是全部交互合同。每个格子区分地面与占位，属性／状态／来源分别显示；时间轴能说明名义计划、覆盖、打断、无目标、非法输入与实际0结果。玩家可以暂停、单步和查原因，战斗中不打开编辑权限。
...
[UX07–08功能清单](yanzhou-rc1/08-journey-ui-and-assets.md)覆盖34词卡、19可换件、3法杖、8敌人、2元素、3环境；树和草地各9种火冰痕迹组合，16类效果变化、12种声音用途和3种音乐情境。
...
| 具体设备兼容、性能与版本兼容 | 制作方案不能偷偷删除规则或破坏重播合同 | [外部开发进度](../../docs/code-development-index.md)；发行前验证 |

## 14. 原型与测试计划

[验收计划与固定输入](yanzhou-rc1/09-validation-and-risks.md)完整保留I0–I3库存、九套配置、720单场计划、36边界输入、20整局计划及V01–20局部结果，另列U01–08真实玩家任务。全部NotRun；本轮不执行新玩法测试。
...
[AC-R-001–007、AC-E-001–002与具体输入](yanzhou-rc1/09-validation-and-risks.md)区分规则与体验。正常胜利时刻目标：普通中位20–40、P90≤60；首领中位40–70、P90≤90。生命损耗中位普通5–15、首领10–30；至少3类同预算可行策略。早结束及一次过高伤害另按RG警报列出，不能靠统计排除残血极端。

本版回归包括全部实体／类型、共享槽／取消、正负结果与费用、火冰来源／环境／形态、镶嵌有限触发、商店／休整／路线、疲劳终局与保存。当前有限测试计划不等于全合法组合穷举，更没有已通过结论。

## 16. 风险、未知项与决策

[风险与责任表](yanzhou-rc1/09-validation-and-risks.md)逐项列读卡、策略支配、护甲等待、来源混淆、资源可达、保存和最终表现。首版规则缺口已关闭；仍须验证的是设计是否达成目标，负责人为相应主设计／UI／QA／玩家研究，首版体验验收前复审。
...
- [七个系统规格](yanzhou-rc1/01-systems.md)
- [构句、法杖、起点、全部角色与效果组](yanzhou-rc1/02-grammar-and-configuration.md)
- [战斗、时间轴、状态、支付与有限触发](yanzhou-rc1/03-combat-and-status.md)
- [元素、空间、宿主、形态与转化](yanzhou-rc1/04-elements-and-environment.md)
- [53实体逐卡目录](yanzhou-rc1/05-card-catalog.md)
- [全参数、获取渠道与经济](yanzhou-rc1/06-parameters-and-economy.md)
- [全路线、敌人、12遭遇与疲劳](yanzhou-rc1/07-route-and-encounters.md)
- [完整旅程、UI、保存、引导和功能资产](yanzhou-rc1/08-journey-ui-and-assets.md)
- [具体验收输入、风险与证据限制](yanzhou-rc1/09-validation-and-risks.md)
- [68正式素材与47inbox的处理记录](yanzhou-rc1/10-source-review.md)
