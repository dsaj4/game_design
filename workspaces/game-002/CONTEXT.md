...
文档角色：Glossary。基准：RC1 / doc.1。术语解释不扩大首版实体许可；详细执行规则见[现行GDD](game-design-workflow/gdd/current/README.md)。
...
| 法杖与镶嵌 | 每根法杖承载一条法术，固定镶嵌定义身份、范围与可选特效；另有2可换槽，本局最多4根出战。 | [对应素材](game-design-workflow/idea-materials/M-2026-09-07-wand-inlay-configuration.md) |
...
| 范围锚点 | 战前从棋盘10格选定的位置参照；不占格、不要求空格，战中固定。固定形状越界裁切，不补足或旋转。 |
...
| 燃烧/冰冻状态 | 附着在宿主上的状态，区别于真实火焰/冰霜生成物；燃烧对应伤害，冰冻按2026-09-13方向增加护甲；具体获得时点、持续及结束效果由词义声明。 |
...
| 邻近脉冲 | 火焰的明示后续环境作用；范围为本格地面及正交邻格，树草不派生同类作用 |

当前动作名称：消耗护甲（原名“吞噬护甲”）。2026-09-13用户要求仅改名，原消耗护甲转执行元素层数的推荐效果保持不变，具体规格以现行内容目录及参数表为准；FX-023保留来源追踪。
...
**阶段名单**：在对应处理阶段开始确定的对象或状态身份范围；处理途中读取当前有效性和当前量，不以新身份替代已失效身份。环境自动名单与刻末状态名单分别确定。

**新元素的首次自动作用**：环境阶段开始前已存在且具备资格的元素可在本刻自动作用；环境阶段中新生最早下刻。该时点不推迟按ST02独立判断的状态处理。

**状态宿主资格**：对象接受某种状态的能力，与它能承受伤害、获得护甲等结果能力分开。火冰资格按[HG01](game-design-workflow/idea-materials/M-2026-09-14-element-status-host-eligibility.md)逐类规定；环境持层不等于环境拥有生命或护甲。

## 首版阵营与资源术语

依据：[G002-CORE-031合格素材](game-design-workflow/idea-materials/M-2026-09-14-factions-wand-ranges-and-starting-loadout.md)。

| 术语 | 含义 |
| --- | --- |
| 阵营 | 宿主身份的友方／敌方／中立归属，独立于固有属性和状态来源；中立不算任一方。 |
| 创建元素的阵营 | 跟随创建方；玩家法杖创建元素为友方，场景预设元素为中立。元素类别不等于“敌人”词卡类别。 |
| 原木杖／木纹芯 | 固定方形范围、在两排棋盘中通常两排三列且裁边，没有额外特效的法杖身份。 |
| 节律杖／节律芯 | 固定十字范围，本杖含简易类型法术的偶数次冷却各减1刻；下限见现行参数表。 |
| 余火杖／余火芯 | 固定横三格范围，对范围内合格燃烧新增量加1的法杖身份。 |
| 起始库存 | 每局同一套7词名12实体、4根原木杖、空可换槽及0金币；生命从预设上限开始，上限见现行参数表。 |
| 出战容量 | 最多4根法杖、一杖一完整法术；替换不会新增槽位，未出战物品不提供被动。 |


## 文档用语

- 三种法杖指款式；四根出战指实体数。
- 55设计ID映射53实体，流派共享词不额外增加副本额度。
- CurrentSpec为现行规则；SourceRecord为来源；HistoricalSnapshot只用于当时判断追溯。
- Closed针对一个有范围的问题；Accepted不意味着验证通过。

[整理前全文](docs/history/pre-organization/CONTEXT.md)
