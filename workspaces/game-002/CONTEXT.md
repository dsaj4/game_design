...
**休整随机卡包**：
选择拿牌后才开启并公开三张互不重名的候选词卡的随机卡包，候选排除本局卡组中同名已达三张上限的词卡；跨卡包仍可出现未满额的已有词。词类分布、卡池权重、稀有度、过滤后词种不足及重抽规则仍待定，来源同休整素材及[不重名确认](game-design-workflow/idea-inbox/2026-09-05-yanzhou-core-combat.md#同一卡包候选不重名确认记录)。
_Avoid_：理解为三个卡包选一个、默认主谓宾各一张，或将同包不重名误解为禁止本局收集副本；也不把休整候选规则自动推广到所有获取场景。
