...
_Avoid_：与每战自动回血、普通词卡奖励或随时可用的恢复能力混称；位置、频率、具体收益和两项都放弃的权限尚未确定。

**休整构筑分支**：
休整中从随机卡包的三张候选词卡中选择一张，收下后加入本局卡组，并放弃本次恢复生命的机会；本局同名普通词卡最多三张的限制继续有效。来源同[休整素材](game-design-workflow/idea-materials/M-2026-09-05-rest-recovery-deck-choice.md)。
_Avoid_：将拿牌等同于必然增强；升级暂不引入，永久删牌或换牌未因此获得采用。

**休整随机卡包**：
休整构筑分支提供的三张随机候选词卡，玩家从中选择一张；卡池、权重、稀有度、候选重复与词类分布、超限候选处理、公开时机、跳过和重抽规则待定。来源同休整素材。
_Avoid_：理解为三个卡包选一个，或默认主谓宾各一张；也不把三选一或普通奖励的跳过权限自动推广到所有获取场景。
