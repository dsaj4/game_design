...
**安全下限**：
为防止抽牌堆和弃牌堆同时为空而设置的本场可循环词卡数量下限；本场移除也不得使该数量低于下限。计数包括手牌、施法区、抽牌堆与弃牌堆，不含已本场移除的词。具体数值与触限处理未定。来源见[卡组下限确认](game-design-workflow/idea-inbox/2026-09-05-deck-vocabulary-cycle.md#卡组下限方向与资格追问)。
