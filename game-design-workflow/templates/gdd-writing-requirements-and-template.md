...
>
> 用途：把已经成形的游戏构思写成一份围绕玩家体验、可供讨论、实现、测试和持续迭代的设计合同。
>
...
2. 复制“六、GDD 正文模板”到新的设计文档，不要直接在本模板中填写项目内容。
3. 保留适用于当前完成度的 `[必填]` 章节；按触发条件保留 `[条件必填]` 章节；删除无关的 `[可选]` 章节。
4. 每个结论都标明证据状态；未知项可以保留，但必须写清负责人、验证方式或决策时点。
5. GDD 中出现正式设定变更时，仍需按本仓库的 `Proposal → Evaluation → Draft Change → Accepted` 流程处理。
...
用于跨策划、程序、美术、音频、QA 和运营协作。除 GDD-1 外必须完成：
...
| 10—13. 引导/呈现/资产/技术 | 不要求 | 原型明确需要时填写 | 必填 |
...
| 交付验收 | 资产、数据、UI、音频、埋点是否齐备 | 关键状态均有视觉与音频反馈 |
...
- 新想法先进入 `Idea` 或 `Proposal`，不能顺手塞进已锁定的 GDD 范围。
...
| 字段 | 类型 | 单位/枚举 | 默认值 | 合法范围 | 空值规则 | 说明 |
| --- | --- | --- | --- | --- | --- | --- |
| id | string | 唯一标识 | 无 | 不重复 | 不允许 | 稳定引用，不随显示名变化 |
...
## 13. 技术与生产约束 `[条件必填：GDD-2]`

| 类别 | 约束/目标 | 测量方式 | 超出时的降级方案 | 负责人 |
| --- | --- | --- | --- | --- |
| 平台/设备 |  |  |  |  |
| 帧率/帧时间 |  |  |  |  |
| 内存/包体 |  |  |  |  |
