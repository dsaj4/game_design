...
> 暂停说明：本 GDD 的地图规则保留，但设计状态已改为 `Parked`。旧战斗 GDD、`combat-lab` 和 `PVE-MAP-01-v0` 已归档，均不得继续实现；恢复条件是新战斗 GDD-0 明确地图输入/输出接口并完成本文件复审。
...
| 创建日期 | `2026-08-22` |
| 最近更新 | `2026-08-22` |
| 目标里程碑 | `Parked：等待新战斗 GDD-0 定义地图接口后重新排期` |
...
| 关联原型/测试 | [旧战斗 GDD-1（Archived）](../../archive/2026-08-22-idiom-combat-system/game-design-workflow/gdd/GDD-2026-08-21-card-battle-system.md)、[`combat-lab v1`（Archived）](../../archive/2026-08-22-idiom-combat-system/combat-lab/README.md)；仅作历史证据，不得实现 |

### 0.1 本版目标

保留 `0.2.0` 的地图灰盒规格和历史决策证据，但停止实现其中依赖旧战斗系统的夹具、接口与验收项。新战斗 GDD-0 形成后，必须重新审查探索血量、PvE 输入/输出、终局和 HUD 字段，递增版本后才能恢复 `Evaluation`。
...
- 固定 AI/PvE 战斗节点的对手来源边界；旧[战斗系统 GDD-1（Archived）](../../archive/2026-08-22-idiom-combat-system/game-design-workflow/gdd/GDD-2026-08-21-card-battle-system.md)接口仅保留作历史记录，当前不接入或实现。
...
| 0.2.1 | 2026-08-22 | Codex | 将文档转为 Parked，停用旧 PvE 夹具和战斗接口 | 用户要求归档旧战斗系统并重新设计 | 文档状态、战斗依赖、原型、验收和恢复条件 |
...
| [攻防共享周期战斗能量](../idea-materials/M-2026-08-21-shared-cycle-combat-energy.md) | 旧战斗与探索移动能量的历史边界 | 3、4、8 | `Park / Historical` | 旧战斗能量已归档，不得成为新接口依赖 |
| [固定 AI/PvE 地图战斗节点](../idea-materials/M-2026-08-22-fixed-pve-map-battles.md) | 战斗对手来源边界 | 0、2–5、9、16 | `Park Pending New Interface` | 固定 PvE/非 PvP 方向保留，具体配置与结果字段等待新战斗 GDD |
...
| 战斗能量（旧方案，已归档） | 旧单局战斗内部按攻防周期刷新的卡牌行动预算，仅用于解释历史规格 | 探索移动能量、长期经济 |
...
### 4.3 SYS-W-003：固定 PvE 地图—战斗接口（历史接口，已停用）

> `PVE-MAP-01-v0` 及本节全部字段来自已归档战斗方案，当前状态为 `Archived / Do Not Implement`。本节只保留接口复审清单；新战斗 GDD 必须替换玩家输入、系统输入、结果、终局和反馈后，本接口才能重新进入验收。
...
| `PVE-MAP-01-v0` | `sheep_core` + `sheep_starter_v0`；`balanced` 启发式 AI；初始/最大血量 12；种子 `2026082201`；12 个完整回合测试上限 | 旧战斗接口的历史夹具，不得继续实现 | `Archived / Do Not Implement` |
...
| 战斗能量 | 旧战斗 GDD 定义的己方回合刷新 | 旧卡牌与组合行动 | 否；不离开战斗 | 仅用于历史复盘 | `Archived External / Replace` |
...
| 原型形式 | `Parked`；原桌面灰盒与旧单机 PvE 入口不再实现，等待新战斗接口复审 |
| 必须实现 | 当前无；地图结构可保留为设计输入，但不得建立旧 PvE、血量往返或战斗能量接口 |
| 明确不实现 | PvP/匹配/联网、程序生成、正式经济与事件、治疗、经营、交易、存档、正式音画和完整引导 |
| 可使用的替代物 | 待新战斗 GDD 确认；不得使用固定羊构筑或旧启发式 AI 代替新系统 |
| 预计成本/周期 | `Unknown / Parked`；原三周计划已暂停 |
| 版本与构建 | GDD `0.2.1 / Parked`；不存在获准实现的当前构建 |
...
| AC-R-W006 | 规则 | 战斗初始化 | 以当前血量进入 `CBT-01` | 载入 `PVE-MAP-01-v0` | 历史验收项；必须由新战斗接口替换 | 战斗程序/QA | `Blocked / Replace` |
| AC-R-W007 | 规则 | 战斗胜利返回 | 玩家剩余血量 >0 | 返回地图 | 历史验收项；结果字段等待新战斗接口 | 战斗/地图程序 | `Blocked / Replace` |
| AC-R-W008 | 规则 | 战斗失败 | 玩家血量归零 | 战斗返回 | 历史验收项；失败条件等待新战斗接口 | 战斗/地图程序 | `Blocked / Replace` |
...
| 正式 PvE 配置与完整战前信息范围 | 难度、风险标签可信度和战斗准备 | `Unknown / Parked`；`PVE-MAP-01-v0` 已归档 | 新战斗 GDD 明确接口后重新建立 PvE 夹具 | 战斗/系统设计 | 地图 GDD 恢复前 |
...
| DEC-W-010 | 2026-08-22 | 暂停本 GDD 及旧 PvE 夹具实现 | 继续沿用旧战斗接口、只替换规则名称 | 战斗系统已重新开放设计，旧接口会预设新方案 | 用户要求归档旧战斗并重新设计 | 新战斗 GDD-0 明确地图接口并完成本文件复审 |

## 17. 附录 `[可选]`

- 统一写作规范：[GDD 写作要求与模板](../templates/gdd-writing-requirements-and-template.md)
- 历史战斗内部规则：[战斗系统 GDD-1（Archived）](../../archive/2026-08-22-idiom-combat-system/game-design-workflow/gdd/GDD-2026-08-21-card-battle-system.md)
- 领域术语：[CONTEXT.md](../../CONTEXT.md)
- 当前研究问题：[current-questions.md](../../research/00-index-and-roadmap/current-questions.md)

## 18. 提交前自检 `[必填]`

- [x] 文档成熟度为 `GDD-1`，设计状态为 `Parked`，旧战斗接口均标记为不可实现。
- [x] 一句话概念包含玩家身份、核心动作、目标、取舍和差异点。
- [x] 设计支柱映射到具体规则，反支柱排除了当前不做的方向。
- [x] 核心循环写清进入、移动、节点处理、战斗、反馈、撤离与失败。
- [x] 探索移动能量继续有效；探索血量与旧战斗能量接口等待新 GDD 复审。
