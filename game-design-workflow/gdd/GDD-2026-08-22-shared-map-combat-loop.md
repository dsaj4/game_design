...
| 战斗节点使用预先配置、可复现的固定 AI/PvE 对手 | `Hypothesis` | 对手来源和原型边界已确认；具体配置待地图—战斗灰盒验证 |
...
- 最小验证方式：制作一张单隐藏撤离点灰盒图；资源/事件使用占位结果，战斗节点接入版本锁定、预先配置的 AI/PvE 对手和现有战斗规则；对同一地图测试少量不同初始能量配置，记录路线、出口发现时点、战后改路和失败归因。
...
| 地图—战斗接口 | 当前节点为战斗节点；玩家探索血量大于 0；节点已绑定版本可追溯的 AI/PvE 对手 | 不进行玩家匹配；将当前探索血量作为玩家战斗初始血量；战斗内部使用独立战斗能量；胜利后返回剩余血量 | PvE 战斗结果、剩余探索血量、战斗节点完成状态 | 具体对手配置、战前信息和战斗外奖励 `Unknown` |
| 撤离与终局 | 位于唯一撤离点，或结束条件被触发 | 起始入口不提供撤离；抵达撤离点后先提供撤离机会，再按血量和能量判定是否可继续；确认撤离则成功结束 | 明确显示成功撤离或失败原因 | 收益/损失 `Parked` |
...
| --- | --- | --- | --- | --- | --- |
| 起始入口 | 开始本次探索，不提供撤离 | 初始化完成 | 普通节点拓扑、相邻普通节点标签、当前状态 | 选择首个相邻节点 | 提交合法移动 |
| 资源节点 | 获取占位节点结果 | 相邻且能量至少 1 点 | 类型与三档风险/收益；具体内容未知 | 是否承担该路线风险 | 占位处理完成 |
| 事件节点 | 接受一次事件处理 | 相邻且能量至少 1 点 | 类型与三档风险/收益；具体事件未知 | 是否进入该分支 | 占位处理完成 |
| 战斗节点 | 通过固定 PvE 战斗解决当前遭遇 | 相邻且能量至少 1 点；节点已绑定可复现 AI/PvE 对手 | 类型、三档风险/收益和 PvE 性质；具体敌方信息范围 `Unknown` | 是否以当前血量进入战斗 | 战斗结束；血量归零则探索失败 |
...
- 系统在几何上保证存活且可继续操作的节点状态存在通往唯一撤离点的相邻路径；返回起始入口不算成功撤离。当前能量是否足以完成该路径是待灰盒验证的设计变量，不被“几何可达”自动保证。
...
| 固定 PvE 对手的具体配置与战前信息范围 | 难度、风险标签可信度和战斗准备 | `Unknown` | GDD-1 锁定对手版本，并比较仅风险档位与追加敌方摘要 | 战斗/系统设计 | 地图灰盒战斗配置冻结前 |
...
| 2026-08-22 | 起始入口只负责开始探索，不能撤离；唯一隐藏撤离点是唯一成功出口 | 起始入口也可撤离、多个出口 | 避免入口成为第二个已知出口，保持单出口与寻找目标一致 | 单出口导致不可接受的无解率或盲猜挫败 |
| 2026-08-22 | 首个地图原型只使用固定 AI/PvE 战斗节点 | 直接接入玩家 PvP、仅使用固定结果 | 排除匹配、联网和真人差异，保留真实战斗—探索血量接口 | 固定 AI 无法提供可用血量压力，或结论被单一 AI 偏差主导 |
